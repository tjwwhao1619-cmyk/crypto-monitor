...
DISCORD_SYMBOL_CONFLICT_WINDOW_SECONDS = 1800
...
VALID_BINANCE_USDT_SYMBOL_RE = re.compile(r"^[A-Z0-9]{1,20}USDT$")
...
DISCORD_CHANNEL_ENV_KEYS = {
    ...
    "main_asset": "DISCORD_MAIN_ASSET_CHANNEL_ID",
...
DISCORD_ROUTE_AUDIT_LIMIT = 20
DISCORD_VISIBLE_DEDUPE_SECONDS = 15 * 60
DISCORD_MAIN_ALLOWED_TITLES = ("🟢 高把握信号", "🟢 实时重点")
DISCORD_MAIN_FORBIDDEN_TITLE_KEYWORDS = (
    "重点观察",
    "爆发观察",
    "高位观察",
    "多空分歧观察",
...
DISCORD_SUMMARY_ONCHAIN_TITLE_KEYWORDS = ("链上", "DEX", "CoinGlass", "稳定币", "外部资金", "数据源", "采集统计")
...
class DiscordOutboundMessage:
    ...
    channel_corrected: bool = False


@dataclass(frozen=True)
class DiscordSymbolSignalMemory:
    timestamp: float
    symbol: str
    side: str
    signal: Signal
...
class DerivativesMonitor:
    def __init__(self, config: dict[str, Any]) -> None:
        ...
        self.discord_signal_cooldowns: dict[str, tuple[float, str]] = {}
        self.discord_symbol_signal_memory: dict[str, DiscordSymbolSignalMemory] = {}
        self.discord_symbol_signal_memory_lock = threading.Lock()
        self.discord_route_audit: deque[dict[str, Any]] = deque(maxlen=DISCORD_ROUTE_AUDIT_LIMIT)
    ...
    def stock_token_channel_key(self) -> str:
        return discord_stock_token_channel_key(self.discord_config.channel_ids)
    ...
    def format_stablecoin_liquidity_radar(self, target: str | None = None) -> str:
        ...
        snapshots = [self.stablecoin_snapshot_from_rows(asset, rows_by_asset.get(asset, [])) for asset in assets]
        lines = [f"稳定币整体：{stablecoin_overall_label([str(snapshot.get('conclusion') or '') for snapshot in snapshots])}"]
        for snapshot in snapshots:
            lines.extend(self.format_stablecoin_liquidity_asset_lines(snapshot, detail=len(assets) == 1))
        lines.append("说明：稳定币供应变化不等于交易所买盘，只作流动性背景。")
        return "\n".join(lines)
    ...
    def format_stablecoin_liquidity_asset_lines(self, snapshot: dict[str, Any], detail: bool = False) -> list[str]:
        ...
        conclusion = str(snapshot.get("conclusion") or "稳定币供应数据不足，等待下次采集")
        short = stablecoin_conclusion_short(conclusion)
        lines = [
            f"{asset}: {format_usd_plain(supply)}，24h {format_percent_optional(changes.get('24h'))}，"
            f"7d {format_percent_optional(changes.get('7d'))}，{short}",
        ]
        if detail:
            chain_limit = 5
            chains_text = stablecoin_chain_distribution_text(chain_top, chain_limit)
            lines.append(f"链分布 Top{chain_limit}: {chains_text}")
    ...
    def send_stablecoin_liquidity_summary_if_due(self) -> None:
        ...
        self.enqueue_discord_message(discord_onchain_embed_v2("🟠 稳定币流动性背景", message, channel_key))
    ...
    def format_coinglass_panel_symbol_lines(self, snapshot: dict[str, Any], detail: bool = False) -> list[str]:
        ...
        if detail:
            human_line = format_coinglass_human_summary_line(snapshot)
            lines = [
                "",
                human_line,
                symbol,
                balance_text.replace("n/a", "暂无数据"),
                oi_text,
                funding_text,
                taker_text,
                orderbook_text.replace("n/a", "暂无数据"),
                f"数据更新时间: {updated}",
                f"综合判断: {judgement}",
            ]
            if cached_line:
            ...
            return lines
        lines = [format_coinglass_human_summary_line(snapshot)]
        if cached_line:
    ...
    def send_coinglass_summary_if_due(self) -> None:
        ...
        self.save_state()
        channel_key = "onchain" if self.discord_config.channel_ids.get("onchain") else "summary"
        self.enqueue_discord_message(discord_onchain_embed_v2("🔷 CoinGlass主流资金", message, channel_key))
    ...
    def stablecoin_overview_lines_and_bias(self) -> tuple[list[str], str]:
        ...
        lines = [f"稳定币整体：{stablecoin_overall_label(conclusions)}"]
        for snapshot in snapshots:
            changes = snapshot.get("changes") if isinstance(snapshot.get("changes"), dict) else {}
            conclusion = str(snapshot.get("conclusion") or "")
            lines.append(
                f"{snapshot.get('asset')}: {format_usd_plain(parse_float(snapshot.get('supply')))}，"
    ...
    def coinglass_overview_lines_and_bias(self) -> tuple[list[str], str]:
        ...
        for symbol in ONCHAIN_SUMMARY_SYMBOLS:
            ...
            if short_bias == "支撑":
                ...
                insufficient += 1
            lines.append(format_coinglass_human_summary_line(snapshot))
    ...
    def external_funds_overview_embed(self, channel_key: str = "onchain") -> DiscordOutboundMessage:
        ...
        fields = [
            ("稳定币背景", discord_field_value("\n".join(stable_lines)), False),
            ("主流币外部资金", discord_field_value("\n".join(coinglass_lines)), False),
            ("链上事件", discord_field_value("\n".join(onchain_lines[:3])), False),
    ...
    def log_signal(self, signal: Signal) -> None:
        ...
        discord_route = discord_route_decision(signal, snapshot, {"load_price_action": False}) if snapshot else RouteDecision("suppress", "静默", "no snapshot", 0, True, ["no_snapshot"])
        ...
        row = {
            ...
            "discord_route": discord_route.route,
            "discord_route_score": discord_route.score,
            "discord_route_reason": discord_route.reason,
    ...
    def notify(self, signal: Signal) -> None:
        ...
        if discord_available:
            self.route_discord_signal(signal, priority, quality_score, quality_reason)
    ...
    def discord_signal_key(self, signal: Signal) -> str:
        return f"{signal.symbol}/{signal.kind}"

    def remove_queued_discord_symbol_messages(self, symbol: str) -> int:
        removed = 0
        target = str(symbol or "").strip().upper()
        with self.discord_outbound_queue.mutex:
            kept = []
            while self.discord_outbound_queue.queue:
                item = self.discord_outbound_queue.queue.popleft()
                if str(item.symbol or "").strip().upper() == target and item.kind != "conflict_observe":
                    removed += 1
                    continue
                kept.append(item)
            self.discord_outbound_queue.queue.extend(kept)
        return removed

    def discord_symbol_conflict_previous(
        self,
        signal: Signal,
        side: str,
        now: float,
    ) -> DiscordSymbolSignalMemory | None:
        if side not in {"long", "risk"}:
            return None
        symbol = str(signal.symbol or "").strip().upper()
        with self.discord_symbol_signal_memory_lock:
            previous = self.discord_symbol_signal_memory.get(symbol)
            if previous and now - previous.timestamp <= DISCORD_SYMBOL_CONFLICT_WINDOW_SECONDS and previous.side != side:
                return previous
        return None

    def remember_discord_symbol_signal(
        self,
        signal: Signal,
        side: str,
        now: float,
        ...
        symbol = str(signal.symbol or "").strip().upper()
        with self.discord_symbol_signal_memory_lock:
            self.discord_symbol_signal_memory[symbol] = DiscordSymbolSignalMemory(
                timestamp=now,
                symbol=symbol,
                side=side,
                signal=signal,
    ...
    def route_discord_signal(
        ...
        if final_decision.route == DISCORD_ROUTE_DIGEST:
            ...
            return
        if self.discord_visible_signal_is_duplicate(signal, final_decision, quality_score, now):
            return
        side = discord_side_for_signal(signal, final_decision)
        previous = self.discord_symbol_conflict_previous(signal, side, now)
        if previous is not None:
            conflict_decision = discord_conflict_route_decision(max(final_decision.score, previous.route_decision.score))
            if self.discord_visible_signal_is_duplicate(signal, conflict_decision, quality_score, now):
                return
            ...
            self.discord_signal_cooldowns[key] = (now, conflict_decision.route)
            self.remember_discord_symbol_signal(
                signal,
                side,
                now,
                priority,
            ...
            logging.info(
                "Discord conflict merged: symbol=%s previous=%s current=%s channel_key=%s",
                signal.symbol,
                previous.side,
                side,
                channel,
            )
            self.record_discord_visible_signal(signal, conflict_decision, quality_score, now)
            return
        ...
        self.remember_discord_symbol_signal(signal, side, now, priority, quality_score, quality_reason, final_decision, channel)
        self.record_discord_visible_signal(signal, final_decision, quality_score, now)

    def discord_visible_signal_record(self, signal: Signal, route_decision: RouteDecision, quality_score: int, now: float) -> dict[str, Any]:
        snapshot = signal.snapshot
        conviction = 0
        risk_score = 0
        if snapshot is not None:
            try:
                ...
                conviction = 0
            try:
                risk_score, _risk_label, _risk_reason = trap_risk_score(snapshot, signal)
            except Exception:
                risk_score = 0
        return {
            "timestamp": now,
            ...
            "route_group": discord_visible_route_group(route_decision.route),
            "direction_group": discord_visible_direction_group(signal, route_decision),
            "kind_family": discord_visible_kind_family(signal.kind, route_decision),
            "kind": signal.kind,
            "quality": int(quality_score or 0),
            "conviction": int(conviction or 0),
            "risk_score": int(risk_score or 0),
            "price_change": snapshot.price_change_percent if snapshot else None,
            "oi_change": snapshot.oi_change_percent if snapshot else None,
        }

    def discord_visible_signal_is_duplicate(
        self,
        signal: Signal,
        route_decision: RouteDecision,
        quality_score: int,
        ...
        if route_decision.route in {DISCORD_ROUTE_SUPPRESS, DISCORD_ROUTE_DIGEST}:
            return False
        current_time = now or time.time()
        current = self.discord_visible_signal_record(signal, route_decision, quality_score, current_time)
        recent = getattr(self, "discord_visible_signal_recent", None)
        if recent is None:
            self.discord_visible_signal_recent = deque(maxlen=500)
            self.discord_visible_signal_lock = threading.Lock()
            recent = self.discord_visible_signal_recent
        lock = getattr(self, "discord_visible_signal_lock", None)
        if lock is None:
            self.discord_visible_signal_lock = threading.Lock()
            lock = self.discord_visible_signal_lock
        with lock:
            previous_items = list(recent)
        for previous in previous_items:
            if current_time - float(previous.get("timestamp") or 0) > DISCORD_VISIBLE_DEDUPE_SECONDS:
                continue
            if previous.get("symbol") != current["symbol"]:
                continue
            if previous.get("route_group") != current["route_group"]:
                continue
            if previous.get("direction_group") != current["direction_group"]:
                continue
            if previous.get("kind_family") != current["kind_family"]:
                continue
            price_delta = abs(float(current.get("price_change") or 0) - float(previous.get("price_change") or 0))
            oi_delta = abs(float(current.get("oi_change") or 0) - float(previous.get("oi_change") or 0))
            if price_delta >= 2 or oi_delta >= 3:
                continue
            if discord_visible_route_level(current["route"]) > discord_visible_route_level(str(previous.get("route") or "")):
                continue
            if int(current.get("risk_score") or 0) - int(previous.get("risk_score") or 0) >= 3:
                continue
            if int(current.get("conviction") or 0) - int(previous.get("conviction") or 0) >= 15:
                continue
            if previous.get("route") != DISCORD_ROUTE_RISK_REALTIME and current.get("route") == DISCORD_ROUTE_RISK_REALTIME:
                continue
            age = int(current_time - float(previous.get("timestamp") or current_time))
            logging.info(
                "Discord duplicate visible signal suppressed: symbol=%s family=%s previous_age=%s quality_old=%s quality_new=%s",
                current["symbol"],
                current["kind_family"],
                age,
                previous.get("quality", "-"),
                current.get("quality", "-"),
            )
            self.record_discord_route_audit(
                DiscordOutboundMessage(
                    channel_key="dedupe_suppress",
                    title=f"duplicate visible suppressed {current['symbol']}",
                    symbol=current["symbol"],
                    kind=current["kind_family"],
                    final_route=current["route"],
                    channel_corrected=True,
                )
            )
            return True
        return False

    def record_discord_visible_signal(
        self,
        signal: Signal,
        route_decision: RouteDecision,
        quality_score: int,
        ...
        if lock is None:
            self.discord_visible_signal_lock = threading.Lock()
            lock = self.discord_visible_signal_lock
        record = self.discord_visible_signal_record(signal, route_decision, quality_score, current_time)
        with lock:
    ...
    def enqueue_discord_suppressed_digest(
        ...
        route = "digest"
        if reason_context:
            route = str(reason_context.get("route") or route)
        ...
        display_reason = clean_discord_reason(str(reason or "") or evidence_summary)
        ...
        logging.info(
            "Discord suppressed digest queued: %s kind=%s route=%s reason=%s",
            item.symbol,
            item.kind,
            item.route,
            item.reason,
    ...
    def enqueue_discord_alt_watch_signal(
        ...
        trap_score, _trap_label, _trap_reason = trap_risk_score(snapshot, signal)
        if route_decision is None and quality_score < 25 and conviction < 45:
            return
        if route_decision is None and trap_score >= 8:
            return
        if route_decision is None and not (
            conviction >= 50
            or quality_score >= 45
            or ev_score >= 6
        ...
        with self.discord_alt_watch_lock:
            ...
            if any(item.symbol == symbol for item in self.discord_alt_watch_queue):
                return
            _short_flow, _mid_flow, _long_flow, flow_label, _flow_reason = flow_horizon_scores(snapshot)
            sort_score = int(route_decision.score if route_decision else conviction + quality_score + ev_score * 3 + leading.leading_score * 3)
            item = DiscordAltWatchItem(
                ...
                reason=format_alt_watch_reason(
                    humanize_route_reason(route_decision, signal) if route_decision else (ev_summary or quality_reason or signal.message)
                ),
                sort_score=sort_score,
                route=route_decision.route if route_decision else "observe",
            )
        ...
        for item in items:
            ...
            lines.append(
                (
                    ...
                    f"OI {format_percent_optional(item.oi_change_percent)} | {humanize_flow_label(item.flow_label)} | "
    ...
    def send_onchain_summary_if_due(self) -> None:
        ...
        self.save_state()
        self.enqueue_discord_message(
            discord_onchain_embed_v2("🧭 主流外部资金观察", message, "onchain")
        )
    ...
    def enqueue_discord_message(self, item: DiscordOutboundMessage) -> bool:
        ...
        if not self.discord_config.bot_token:
            return False
        initial_channel = item.channel_key
        selected_channel = discord_channel_for_payload(
            item.kind,
            route_decision=RouteDecision(
                item.final_route,
                discord_route_level(item.final_route),
                "",
                0,
                item.final_route not in {DISCORD_ROUTE_REALTIME, DISCORD_ROUTE_RISK_REALTIME},
                item.route_tags or [],
            ) if item.final_route else None,
            signal=None,
            title=item.title,
            tags=item.route_tags,
            channel_ids=self.discord_config.channel_ids,
            symbol=item.symbol,
            kind=item.kind,
        )
        if selected_channel not in {"suppress", "command"}:
            if selected_channel != item.channel_key:
                if selected_channel in {"main_asset", "alerts"} and discord_is_main_asset_signal(
                    item.symbol,
                    item.kind,
                    item.title,
                    item.final_route,
                    ...
                    logging.warning(
                        "Discord main asset channel mismatch corrected: symbol=%s kind=%s final_route=%s from=%s to=%s title=%s",
                        item.symbol or "-",
                        item.kind or "-",
                        item.final_route or "-",
                        item.channel_key,
                ...
                elif selected_channel == "onchain":
                    logging.warning(
                        "Discord onchain channel mismatch corrected: title=%s from=%s to=%s",
                        item.title or "-",
                        item.channel_key,
                        selected_channel,
                    )
                elif "多空分歧观察" in str(item.title or ""):
                    logging.warning(
                        "Discord conflict channel mismatch corrected: symbol=%s kind=%s final_route=%s from=%s to=%s title=%s",
                        item.symbol or "-",
                        item.kind or "-",
                        item.final_route or DISCORD_ROUTE_CONFLICT_OBSERVE,
                        item.channel_key,
                        selected_channel,
                        item.title or "-",
                    )
            ...
            item = replace(item, channel_key=selected_channel)
        ...
        corrected = corrected_channel != initial_channel or item.channel_key != initial_channel
        if corrected_channel != item.channel_key:
            item = replace(item, channel_key=corrected_channel, channel_corrected=True)
        elif corrected:
            item = replace(item, channel_corrected=True)
        ...
        try:
            ...
            self.record_discord_route_audit(item)
            logging.info(
                "Discord enqueue: payload_type=%s symbol=%s kind=%s final_route=%s channel_key=%s channel_id=%s title=%s",
                item.kind or "-",
                item.symbol or "-",
                item.kind or "-",
                item.final_route or "-",
    ...
    def record_discord_route_audit(self, item: DiscordOutboundMessage) -> None:
        ...
        if audit is None:
            self.discord_route_audit = deque(maxlen=DISCORD_ROUTE_AUDIT_LIMIT)
            self.discord_route_audit_lock = threading.Lock()
            audit = self.discord_route_audit
        lock = getattr(self, "discord_route_audit_lock", None)
        if lock is None:
            self.discord_route_audit_lock = threading.Lock()
            lock = self.discord_route_audit_lock
        with lock:
            audit.appendleft(
                {
                    "time": time.time(),
                    "title": item.title or item.kind or item.symbol or "-",
                    "final_route": item.final_route or "-",
                    "channel_key": item.channel_key,
                    "corrected": bool(item.channel_corrected),
                }
            )
    ...
    def enqueue_discord_status(self, title: str, message: str) -> None:
        ...
        self.enqueue_discord_message(replace(discord_summary_embed_v2(title, message, channel_key), kind="status"))
    ...
    def discord_command_response(self, text: str) -> str | DiscordOutboundMessage:
        ...
        if command == "!频道":
            return self.format_discord_channel_config_response()
        if command == "!频道审计":
            return self.format_discord_channel_audit_response()
        if command == "!摘要":
        ...
        if command == "!coinglass":
            target = parts[1] if len(parts) >= 2 else None
            title = f"🔷 CoinGlass主流资金 {normalize_usdt_symbol(target)}" if target else "🔷 CoinGlass主流资金"
            return discord_onchain_embed_v2(title, self.format_coinglass_panel(target), "onchain")
        if command in ("!链上摘要", "!外部资金摘要"):
            return discord_onchain_embed_v2("🧭 主流外部资金观察", self.format_onchain_summary_from_cache(), "onchain")
    ...
    def format_discord_channel_audit_response(self) -> str:
        audit = getattr(self, "discord_route_audit", None)
        if not audit:
            return "最近暂无 Discord enqueue 审计记录。"
        lock = getattr(self, "discord_route_audit_lock", None)
        if lock is None:
            rows = list(audit)
        else:
            with lock:
                rows = list(audit)
        lines = ["最近 Discord enqueue 路由审计"]
        for row in rows[:DISCORD_ROUTE_AUDIT_LIMIT]:
            ts = format_ts_short(row.get("time"))
            title = truncate_text(str(row.get("title") or "-"), 60)
            lines.append(
                f"{ts} | {title} | final_route={row.get('final_route') or '-'} | "
                f"channel_key={row.get('channel_key') or '-'} | corrected={'yes' if row.get('corrected') else 'no'}"
            )
        return "\n".join(lines)
    ...
    def format_discord_route_candidates_response(
        ...
        try:
            ...
            logging.exception("Failed to read Discord route candidate rows")
        ...
        for recent_index, row in enumerate(rows):
            ...
            if not is_valid_binance_usdt_symbol(symbol):
                continue
            normalized_kind = topq_kind_normalized(row.get("kind"))
            price_action = (
                light_multi_timeframe_price_action(symbol)
                if normalized_kind in {"discovery", "bottom_reversal"}
                else None
            )
            ...
            side = discord_side_for_signal(row, decision)
            raw_entries.append((symbol, side, display_route, score_key, row, decision, price_action))

        sides_by_symbol: dict[str, set[str]] = {}
        for symbol, side, _display_route, _score_key, _row, _decision, _price_action in raw_entries:
            if side in {"long", "risk"}:
                sides_by_symbol.setdefault(symbol, set()).add(side)
        conflict_symbols = {symbol for symbol, sides in sides_by_symbol.items() if {"long", "risk"}.issubset(sides)}
        conflict_added: set[str] = set()
        for symbol, side, display_route, score_key, row, decision, price_action in raw_entries:
            if symbol in conflict_symbols:
                if symbol in conflict_added:
                    continue
...
def discord_channel_for_signal(signal: Signal, priority: str) -> str:
    ...
    if signal.symbol in MAINSTREAM_WATCH_SYMBOLS or normalized_kind in {"main_momentum_watch", "main_trend_watch", "main_risk_watch"}:
        return "main_asset"
    if normalized_kind in {"top_risk", "top_exhaustion", "distribution"}:
        return "risk"
...
def discord_channel_for_route(
    ...
    return discord_channel_for_payload("signal", route_decision=route_decision, signal=signal, channel_ids=channel_ids)


def discord_observe_channel_key(channel_ids: dict[str, str] | None = None) -> str:
...
def discord_main_asset_channel_key(channel_ids: dict[str, str] | None = None) -> str:
    channel_ids = channel_ids or {}
    return "main_asset" if channel_ids.get("main_asset") else "alerts"


def discord_is_main_asset_signal(
    symbol: str | None = None,
    kind: str | None = None,
    title: str | None = None,
    route: str | None = None,
) -> bool:
    if route in {DISCORD_ROUTE_DIGEST, DISCORD_ROUTE_SUPPRESS}:
        return False
...
def discord_visible_route_group(route: str | None) -> str:
    value = str(route or "")
    if value == DISCORD_ROUTE_RISK_REALTIME:
        return "risk_realtime"
    if value == DISCORD_ROUTE_REALTIME:
        return "realtime"
    if value == DISCORD_ROUTE_PRIORITY_OBSERVE:
        return "priority_observe"
    if value in {DISCORD_ROUTE_OBSERVE, DISCORD_ROUTE_CONFLICT_OBSERVE}:
        return "observe"
    if value == DISCORD_ROUTE_DIGEST:
        return "digest"
    return value or "unknown"


def discord_visible_route_level(route: str | None) -> int:
    group = discord_visible_route_group(route)
    return {
        "digest": 0,
        "observe": 1,
        "priority_observe": 2,
...
def discord_visible_kind_family(kind: str | None, route_decision: RouteDecision | None = None) -> str:
    normalized = topq_kind_normalized(kind)
    route = route_decision.route if route_decision else ""
    if normalized in DISCORD_RISK_KINDS:
        return "risk_family"
    if normalized in {"bottom_reversal", "discovery", "hot_breakout", "main_momentum_watch", "main_trend_watch"}:
        return "long_family"
    if normalized == "conflict_observe" or route == DISCORD_ROUTE_CONFLICT_OBSERVE or discord_is_conflict_observe(route_decision):
        return "conflict_family"
    if route == DISCORD_ROUTE_DIGEST:
        return "digest_family"
    return f"{normalized or 'unknown'}_family"


def discord_visible_direction_group(signal: Signal, route_decision: RouteDecision | None = None) -> str:
    if discord_is_conflict_observe(route_decision):
        return "conflict"
    side = discord_side_for_signal(signal, route_decision)
    return side or signal_direction_label(signal.kind) or "unknown"


def discord_channel_for_payload(
    ...
    channel_ids = channel_ids or {}
    payload = str(payload_type or "").strip().lower().replace("-", "_").replace(" ", "_")
    route = route_decision.route if route_decision else DISCORD_ROUTE_OBSERVE
    normalized_kind = topq_kind_normalized(signal.kind if signal else kind or payload)
    tag_set = set(tags or (route_decision.route_tags if route_decision else []) or [])
    title_text = str(title or "")
    text = " ".join([payload, normalized_kind, route, title_text, " ".join(tag_set)])
    payload_symbol = signal.symbol if signal else symbol

    if payload in {"command", "diagnosis"}:
    ...
    if payload in {"onchain", "external_funds_overview", "futures_only_liquidity"} or text_has_any(
        ...
        return "onchain"
    if payload in {"suppressed_digest", "quiet_digest"} or route == DISCORD_ROUTE_DIGEST or "静默" in title_text:
        return "digest"
    if payload == "status":
        if "摘要" in title_text or "简报" in title_text:
            return "summary"
        return "debug"
    if payload == "summary":
        if text_has_any(title_text, DISCORD_SUMMARY_ONCHAIN_TITLE_KEYWORDS):
            return "onchain"
...
def discord_correct_route_channel_mismatch(
    ...
    channel_ids = channel_ids or {}
    title_text = str(title or "")
    resolved_symbol = symbol or (signal.symbol if signal else None)
    resolved_kind = kind or (signal.kind if signal else None)
    if text_has_any(title_text, DISCORD_SUMMARY_ONCHAIN_TITLE_KEYWORDS) and channel_key != "onchain":
        logging.warning(
            "Discord onchain channel mismatch corrected: title=%s from=%s to=onchain",
            title or "-",
            channel_key,
        )
        return "onchain"
    if discord_is_main_asset_signal(resolved_symbol, resolved_kind, title_text, route) and channel_key not in {"main_asset", "alerts"}:
        ...
        return corrected
    if channel_key == "summary" and "美股代币" in title_text:
        corrected = discord_stock_token_channel_key(channel_ids)
        logging.warning(
            "Discord summary channel mismatch corrected: symbol=%s kind=%s final_route=%s from=%s to=%s title=%s",
            resolved_symbol or "-",
            resolved_kind or "-",
            route or "-",
            channel_key,
    ...
    if "多空分歧观察" in title_text and channel_key in {"main", "high-confidence", "high_confidence", "risk", "digest", "alt_watch"}:
        ...
        return corrected
    if channel_key == "main" and (
        text_has_any(title_text, DISCORD_MAIN_FORBIDDEN_TITLE_KEYWORDS)
        or (title_text and not title_text.startswith(DISCORD_MAIN_ALLOWED_TITLES))
        or route != DISCORD_ROUTE_REALTIME
    ):
        corrected = discord_observe_channel_key(channel_ids)
        logging.warning(
            "Discord main channel mismatch corrected: symbol=%s kind=%s final_route=%s from=%s to=%s title=%s",
            resolved_symbol or "-",
            resolved_kind or "-",
            route or "-",
            channel_key,
...
def discord_channel_for_pending_signal(pending: PendingTelegramSignalMerge) -> str:
    ...
    for signal in pending.signals:
        ...
        if channel in {"main_asset", "main", "risk"}:
            return channel
    ...
    if route_decision:
        ...
        if route_decision.route == DISCORD_ROUTE_PRIORITY_OBSERVE:
            if "breakout_watch" in route_decision.route_tags or "breakout_watch_fallback" in route_decision.route_tags:
                return f"🟡 爆发观察 {pair}"
            return f"🟡 重点观察 {pair}"
        if route_decision.route == DISCORD_ROUTE_OBSERVE:
            return f"🟡 观察信号 {pair}"
        if route_decision.route == DISCORD_ROUTE_RISK_REALTIME:
            return f"🔴 风险重点 {pair}"
        if route_decision.route == DISCORD_ROUTE_REALTIME:
...
def discord_leading_field_value(snapshot: MarketSnapshot, signal: Signal | None) -> str:
    ...
    if not items:
        items = ["- 暂无明显领先信号"]
    return discord_field_value(
        f"{leading.leading_score}/10 | 领先信号：{humanize_leading_direction(leading.leading_direction)} | {leading.leading_label}\n"
        + "\n".join(items)
    )
...
def discord_conflict_aware_summary(
    ...
    if normalized_kind in DISCORD_BULLISH_KINDS and risk_evidence:
        if text_has_any(summary, ("高位拥挤", "大户", "散户", "多头拥挤")):
            return "大户/散户多头拥挤，追高风险升高"
        if text_has_any(summary, ("中长线资金不支持", "资金不支持", "流出")):
            return "短线买盘强，但4h/12h资金仍流出"
        return "资金分歧，等回踩承接确认"
...
def discord_signal_embed_v2(
    ...
    if discord_is_conflict_observe(route_decision):
        return discord_conflict_observe_embed(signal, priority, quality_score, quality_reason, channel_key, route_decision)
    normalized_kind = topq_kind_normalized(signal.kind)
    if normalized_kind == "main_momentum_watch":
        return discord_main_momentum_watch_embed_v2(signal, priority, quality_score, quality_reason, channel_key, route_decision)
    if normalized_kind in {"main_risk_watch", "top_risk", "top_exhaustion", "distribution"} or is_risk_structure_kind(signal.kind):
        return discord_main_risk_watch_embed_v2(signal, priority, quality_score, quality_reason, channel_key, route_decision)
    return discord_realtime_signal_embed_v2(signal, priority, quality_score, quality_reason, channel_key, route_decision)
...
def discord_main_momentum_watch_embed_v2(
    ...
    return DiscordOutboundMessage(
        channel_key=channel_key,
        title=discord_signal_title_for_route(signal, priority, route_decision),
...
def stablecoin_overall_label(conclusions: list[str]) -> str:
    if any("扩张" in item for item in conclusions):
        return "扩张"
    if any("收缩" in item for item in conclusions):
        return "收缩"
    if conclusions and all("数据不足" in item for item in conclusions):
        return "数据不足"
    return "中性"
...
def discord_suppressed_digest_embed_v2(
    ...
    for item in display_items:
        lines.append(
            ...
            f"把握{item.conviction} | 路由={discord_route_human_label(item.route)} | 原因={clean_discord_reason(item.reason)}"
        )
        lines.append(
            f"{format_percent_optional(item.price_change)} / OI{format_percent_optional(item.oi_change)} | "
            f"{humanize_flow_label(item.flow_label)} | {clean_discord_reason(item.evidence_summary or '-')}"
        )
...
def discord_diagnosis_embed_v2(
    ...
    route_decision = discord_route_decision(signal, snapshot, {"price_action": diagnosis_price_action, "load_price_action": False})
    fields = discord_signal_fields(signal, "-", 0, "诊断", route_decision)
...
def discord_help_text() -> str:
    return (
        ...
        "!链上摘要 - 旧别名，当前不代表真实钱包流\n"
        "!诊断 BTCUSDT - 单币诊断简版\n"
        "!频道 - 查看 Discord 频道配置和路由矩阵\n"
        "!频道审计 - 查看最近20条 Discord enqueue 路由\n"
        "!质量 - 信号质量统计\n"
        "!静默 - 等同 /quiet status\n"
...
def format_coinglass_balance_human(balances: dict[str, Any]) -> str:
    parts = []
    for period, label in (("24h", "1d"), ("7d", "7d"), ("30d", "30d")):
        value = format_coinglass_balance_snapshot(balances.get(period))
        if value != "n/a":
            parts.append(f"{label} {value}")
    return "，".join(parts) if parts else "暂无数据"


def coinglass_orderbook_human_from_metrics(metrics: dict[str, Any]) -> str:
    judgement = coinglass_orderbook_judgement(metric_value(metrics, "orderbook_bid_ask_ratio"))
...
def coinglass_human_judgement(metrics: dict[str, Any], balances: dict[str, Any]) -> str:
    ...
    if "均衡" in orderbook:
        return "中性/分歧"
    return coinglass_judgement_short(judgement)


def format_coinglass_human_summary_line(snapshot: dict[str, Any]) -> str:
    symbol = str(snapshot.get("symbol") or "-")
    metrics = snapshot.get("metrics") if isinstance(snapshot.get("metrics"), dict) else {}
    balances = snapshot.get("balances") if isinstance(snapshot.get("balances"), dict) else {}
...
def coinglass_balance_text_from_context(coinglass_text: str | None) -> str:
    ...
    return balance_text or "暂无数据"


def coinglass_orderbook_text_from_context(coinglass_text: str | None) -> str:
    orderbook_text = extract_labeled_segment(str(coinglass_text or ""), "CoinGlass订单簿: ", "\n") or ""
    if "上方卖压偏强" in orderbook_text:
        return "上方卖压偏强"
    if "下方承接偏强" in orderbook_text:
        return "下方承接偏强"
    if "买卖盘相对均衡" in orderbook_text:
        return "买卖盘均衡"
    return "暂无数据"


def external_funds_human_bias(coinglass_text: str | None, spot_score: int) -> str:
    ...
    orderbook = coinglass_orderbook_text_from_context(coinglass_text)
    balance_up = any(value is not None and value > 0 for value in (balance_1d, balance_7d))
    balance_down = any(value is not None and value < 0 for value in (balance_1d, balance_7d))
    sell_pressure = (sell_ratio is not None and sell_ratio > 52) or "卖压" in orderbook
    buy_support = (buy_ratio is not None and buy_ratio > 52) or "承接" in orderbook
    if balance_up and sell_pressure:
        return "外部资金偏风险"
    if balance_down and buy_support:
        return "外部资金偏支撑"
    if orderbook == "上方卖压偏强":
        return "外部资金偏谨慎"
    if orderbook == "买卖盘均衡":
        return "外部资金中性/分歧"
    if coinglass_balance_text_from_context(coinglass_text) == "暂无数据" and spot_score == 5:
        return "外部数据不足"
    return "外部资金中性/分歧"


def external_funds_human_conclusion(coinglass_text: str | None, spot_score: int) -> str:
    bias = external_funds_human_bias(coinglass_text, spot_score)
    orderbook = coinglass_orderbook_text_from_context(coinglass_text)
    if bias == "外部资金偏风险":
        return "抛压仍在，追多需等卖压减弱"
    if bias == "外部资金偏支撑":
        return "外部资金有支撑，但仍需等盘面确认"
    if bias == "外部数据不足":
        return "外部数据不足，暂不提供方向"
    if orderbook == "上方卖压偏强":
        return "上方卖压仍在，追多需谨慎"
    return "买卖盘分歧，等待更明确确认"
...
def format_onchain_brief(
    ...
    balance_text = coinglass_balance_text_from_context(cg_text)
    taker_24h = extract_labeled_segment(cg_text, "主动买卖 24h", "；")
    taker_7d = extract_labeled_segment(cg_text, "CoinGlass主动买卖 7d", "；")
    orderbook_text = coinglass_orderbook_text_from_context(cg_text)
    conclusion = onchain_conclusion(snapshot, spot_score, div_score, cg_text)
    human_bias = external_funds_human_bias(cg_text, spot_score)
    human_conclusion = external_funds_human_conclusion(cg_text, spot_score)
    source_text = external_confirmation_source_text(resolved_spot_text, cg_text)

    if compact:
        return discord_external_data_text(
            f"{snapshot.symbol.replace('USDT', '')}：{human_bias}\n"
            f"- 交易所余额：{balance_text}\n"
            f"- 订单簿：{orderbook_text}\n"
            f"- 结论：{human_conclusion}"
        )

    taker_text = " / ".join(part for part in (taker_24h, taker_7d) if part) or "暂无数据"
    lines = [
        f"结论: {human_bias} - {human_conclusion}",
        ...
        f"CoinGlass主动买卖: {taker_text}",
        f"CoinGlass订单簿: {orderbook_text}",
        source_text,
        f"原始综合判断: {conclusion.replace('数据不足', '数据不足：CoinGlass/现货DEX/订单簿有效项不足')}",
    ]
    return discord_external_data_text("\n".join(lines))
...
def discord_route_value(source: Signal | dict[str, Any], key: str, default: Any = None) -> Any:
...
def discord_route_float(source: Signal | dict[str, Any], key: str, default: float = 0.0) -> float:
    value = parse_float(discord_route_value(source, key))
    return default if value is None else value


def discord_route_metrics(
    signal: Signal | dict[str, Any],
    snapshot: MarketSnapshot | None = None,
    price_action: MultiTimeframePriceAction | None = None,
) -> dict[str, Any]:
    if isinstance(signal, Signal):
        snapshot = snapshot or signal.snapshot
        if snapshot is None:
            return {
                "symbol": signal.symbol,
        ...
        conviction, _conv_label, _conv_reason = conviction_score(snapshot, signal)
        priority, quality, _quality_reason = signal_priority(signal, snapshot)
        leading = leading_signal_score(snapshot, signal)
        ...
        return {
            ...
            "price_change_percent": float(snapshot.price_change_percent or 0),
            "entry_score": int(entry_score or 0),
            "entry_label": str(entry_label or ""),
            "trap": int(trap_score or 0),
            ...
            "kline_short_score": int(price_action.short_score if price_action else 0),
            "kline_mid_score": int(price_action.mid_score if price_action else 0),
            "kline_long_score": int(price_action.long_score if price_action else 0),
            "risk_score": int(trap_score or 0),
        }

    kind = topq_kind_normalized(signal.get("kind"))
    pa = price_action
    kline_label = str(signal.get("kline_label") or "")
    kline_text = kline_label
    if pa is not None:
        kline_text = " ".join([pa.label, pa.recommendation, *pa.items, *pa.risk_items, *pa.patterns])
    return {
        "symbol": str(signal.get("symbol") or "").strip().upper(),
        "kind": kind,
        "conviction": int(discord_route_float(signal, "conviction_score", 0)),
        "priority": str(signal.get("signal_priority") or "-").upper(),
        ...
        "price_change_percent": discord_route_float(signal, "price_change_percent", 0),
        "entry_score": int(discord_route_float(signal, "entry_timing_score", 0)),
        "entry_label": str(signal.get("entry_timing_label") or ""),
        "trap": int(discord_route_float(signal, "trap_risk_score", 0)),
        ...
        "kline_text": kline_text,
        "kline_short_score": int(discord_route_float(signal, "kline_short_score", pa.short_score if pa is not None else 0)),
        "kline_mid_score": int(discord_route_float(signal, "kline_mid_score", pa.mid_score if pa is not None else 0)),
        "kline_long_score": int(discord_route_float(signal, "kline_long_score", pa.long_score if pa is not None else 0)),
        "risk_score": discord_route_float(signal, "risk_score", discord_route_float(signal, "trap_risk_score", 0)),
    }


def discord_price_action_bullish_confirmed(price_action: MultiTimeframePriceAction | None) -> bool:
    if price_action is None:
        return False
    return price_action.score >= 6 and price_action.mid_score >= 5 and not price_action_discord_downgrade(price_action)


def discord_price_action_bottom_confirmed(price_action: MultiTimeframePriceAction | None) -> bool:
    if price_action is None:
        return False
    text = " ".join([price_action.label, price_action.recommendation, *price_action.items, *price_action.patterns])
    return price_action.score >= 5 and text_has_any(text, ("底部", "长下影", "阳包阴", "回踩", "承接", "支撑"))


def discord_price_action_score_confirmed(price_action: MultiTimeframePriceAction | None) -> bool:
    if price_action is None:
        return False
    return max(price_action.score, price_action.short_score, price_action.mid_score, price_action.long_score) >= 6


def discord_price_action_observe_blocked(price_action: MultiTimeframePriceAction | None) -> bool:
    if price_action is None:
        return False
    text = " ".join([price_action.label, price_action.recommendation, *price_action.items, *price_action.risk_items, *price_action.patterns])
    return text_has_any(text, ("明显压力位", "压力位附近", "追高风险"))


def discord_priority_observe_candidate(data: dict[str, Any], risk_kind: bool) -> bool:
    kind = data["kind"]
    if kind == "hot_breakout" or kind not in {"discovery", "bottom_reversal", "top_risk", "top_exhaustion", "main_momentum_watch"}:
        return False
    conviction = int(data["conviction"] or 0)
    if conviction < 50 or conviction > 64:
        return False
    leading = int(data["leading"] or 0)
    evidence = int(data["evidence"] or 0)
    price_action = data.get("price_action")
    if not (leading >= 6 or evidence >= 8 or discord_price_action_score_confirmed(price_action)):
        return False
    evidence_summary = str(data.get("evidence_summary") or "")
    evidence_direction = str(data.get("evidence_direction") or "")
    flow_label = str(data.get("flow_label") or "")
    if risk_kind and not (
        (evidence_direction == "看空/风险" and evidence >= 5)
        or (evidence_direction == "观察" and leading >= 7 and flow_label in {"中长线派发", "资金分歧"})
    ):
        return False
    if not risk_kind and text_has_any(evidence_summary, ("严重追高风险", "高位拥挤")):
        return False
    if not risk_kind and discord_price_action_observe_blocked(price_action):
        return False
    return True


def discord_priority_observe_chase_risk(data: dict[str, Any]) -> bool:
    if data.get("kind") != "discovery":
        return False
    price_action = data.get("price_action")
    price_action_text = ""
    if price_action is not None:
        price_action_text = " ".join([price_action.label, price_action.recommendation, *price_action.items, *price_action.risk_items, *price_action.patterns])
    evidence_summary = str(data.get("evidence_summary") or "")
    price_change = float(data.get("price_change_percent") or 0)
    return (
        str(data.get("flow_label") or "") == "短强中弱"
        or text_has_any(evidence_summary, ("高位拥挤", "谨慎追", "不宜追"))
...
def discord_breakout_watch_candidate(data: dict[str, Any]) -> bool:
    kind = data.get("kind")
    if kind not in {"discovery", "bottom_reversal"}:
        return False
    ...
    kline_text = str(data.get("kline_text") or data.get("kline_label") or "")
    evidence_summary = str(data.get("evidence_summary") or "")
    flow_label = str(data.get("flow_label") or "")
    entry_label = str(data.get("entry_label") or "")
    oi_change = float(data.get("oi_change_percent") or 0)

    if text_has_any(evidence_summary, ("高位拥挤", "注意出货", "不宜追", "谨慎追")):
        return False
    if text_has_any(flow_label, ("中长线派发",)):
        return False
    if trap >= 7:
        return False
    if text_has_any(kline_text, ("长上影", "放量滞涨", "日线压力", "周线压力")):
        return False

    structure_breakout = text_has_any(kline_text, ("短中线突破延续", "短中线转强", "4h延续强", "突破"))
    condition_a = leading >= 6 and short_score >= 8 and structure_breakout and trap <= 6
    condition_b = (
        conviction < 50
        and evidence >= 8
        and flow_label in {"短强中弱", "资金分歧"}
        and oi_change > 0
        and short_score >= 8
    )
    condition_c = (
        text_has_any(entry_label, ("启动前", "启动初期", "启动观察"))
        and trap <= 5
        and short_score >= 7
        and mid_score >= 6
    )
...
def discord_breakout_watch_fallback_candidate(data: dict[str, Any]) -> bool:
    kind = data.get("kind")
    if kind not in {"discovery", "bottom_reversal", "hot_breakout"}:
        return False

    leading = int(data.get("leading") or 0)
    evidence = int(data.get("evidence") or 0)
    entry_score = int(data.get("entry_score") or 0)
    if not (leading <= 3 and evidence < 8 and entry_score < 8):
        return False

    trap = int(data.get("trap") or 0)
    risk_score = float(data.get("risk_score") or trap or 0)
    short_score = int(data.get("kline_short_score") or 0)
    mid_score = int(data.get("kline_mid_score") or 0)
    kline_text = str(data.get("kline_text") or data.get("kline_label") or "")
    evidence_summary = str(data.get("evidence_summary") or "")
    flow_label = str(data.get("flow_label") or "")
    price_change = max(
        float(data.get("price_change_percent") or 0),
        float(data.get("confirm_price_change_percent") or 0),
    )
    oi_change = max(
        float(data.get("oi_change_percent") or 0),
        float(data.get("confirm_oi_change_percent") or 0),
    )
    ...
    structure_ok = (
        (short_score >= 8 and mid_score >= 6)
        or text_has_any(kline_text, ("短中线突破延续", "短中线转强", "4h延续强"))
        or text_has_any(kline_text, ("突破近20根高点", "放量阳线", "连续收盘抬高"))
    )
    if not structure_ok:
        return False

    if kind == "hot_breakout":
        return short_score >= 8 and mid_score >= 7 and price_change >= 4 and oi_change >= 5 and trap <= 4
    return price_change >= 2 and oi_change >= 3


def discord_route_downgrade(route: str) -> str:
    if route in {DISCORD_ROUTE_REALTIME, DISCORD_ROUTE_RISK_REALTIME}:
        return DISCORD_ROUTE_PRIORITY_OBSERVE
    if route == DISCORD_ROUTE_PRIORITY_OBSERVE:
        return DISCORD_ROUTE_OBSERVE
    if route == DISCORD_ROUTE_OBSERVE:
        return DISCORD_ROUTE_DIGEST
    return route


def discord_route_level(route: str) -> str:
    if route == DISCORD_ROUTE_RISK_REALTIME:
        return "风险"
    if route == DISCORD_ROUTE_REALTIME:
        return "重点"
...
DISCORD_ROUTE_HUMAN_LABELS = {
    DISCORD_ROUTE_REALTIME: "实时重点",
    DISCORD_ROUTE_PRIORITY_OBSERVE: "重点观察",
    DISCORD_ROUTE_OBSERVE: "观察",
    DISCORD_ROUTE_DIGEST: "静默摘要",
    DISCORD_ROUTE_RISK_REALTIME: "风险重点",
    DISCORD_ROUTE_SUPPRESS: "已过滤",
}


def discord_route_human_label(route: str | None) -> str:
    return DISCORD_ROUTE_HUMAN_LABELS.get(str(route or ""), "观察")


def discord_route_display_label(route_decision: RouteDecision | None) -> str:
    if route_decision and (
        "breakout_watch" in route_decision.route_tags
        or "breakout_watch_fallback" in route_decision.route_tags
    ):
        return "priority_observe / 爆发观察"
    return discord_route_human_label(route_decision.route if route_decision else None)


def humanize_flow_label(flow_label: str | None) -> str:
    label = str(flow_label or "").strip()
    if label == "短强中弱":
        return "短线买盘增强，但中长周期未确认"
    if label == "短弱中强":
        return "短线偏弱，中长周期仍有支撑"
    if label == "资金分歧":
        return "资金分歧，等待方向确认"
    if label == "多周期共振流入":
        return "多周期资金共振流入"
    if label == "中长线派发":
        return "中长周期资金转弱，注意派发风险"
    if label == "中长线吸筹":
        return "中长周期有吸筹迹象"
    return label or "资金方向不明确"


def humanize_leading_direction(direction: str | None) -> str:
    value = str(direction or "").strip().lower()
    if value in {"long", "bullish", "看多"}:
        return "偏多"
    if value in {"short", "bearish", "risk", "看空", "看空/风险"}:
        return "偏风险"
    return "方向不明确"


def humanize_kline_summary(price_action: MultiTimeframePriceAction | None) -> str:
    if price_action is None or price_action.label == "K线数据不足":
        return "暂无多周期K线确认，仅资金/OI观察"
    label = str(price_action.label or "")
    if text_has_any(label, ("大周期未确认", "箱体未突破")):
        headline = "短线有变化，但大周期未确认"
    elif text_has_any(label, ("追高风险", "压力位附近")):
        headline = "接近压力区，追高风险升高"
    elif price_action.short_score >= 6 and price_action.mid_score < 5:
        headline = "短线转强，中线尚未确认"
    elif price_action.mid_score >= 6 and price_action.long_score < 5:
        headline = "中短线转强，大周期仍需确认"
    elif price_action.score >= 6:
        headline = "多周期结构偏强"
    else:
        headline = label or "K线结构仍需确认"
    details: list[str] = []
    if price_action.short_score >= 6:
        details.append("15m/1h 转强")
    if price_action.mid_score < 5:
        details.append("4h 尚未有效确认")
    if price_action.long_score < 5:
        details.append("日线/周线仍偏弱")
    if price_action.patterns:
        details.extend(
            compact_price_action_pattern(pattern)
            for pattern in price_action.patterns
            if pattern and "数据不足" not in pattern
        )
    elif price_action.risk_items:
        details.extend(str(item) for item in price_action.risk_items if item and "数据不足" not in str(item))
    details = details[:4]
    explain = "；".join(dict.fromkeys(details))
    return f"K线 {price_action.score}/10 | {headline}" + (f"\n解释：{explain}" if explain else "")


def clean_discord_reason(text: str | None) -> str:
    value = str(text or "").strip()
    if not value:
        return "等待资金和K线确认"
    replacements = {
        "conviction 50-64 kept by leading/evidence/Kline": "中等把握，但领先/证据有亮点",
        "conviction 50-64 digest": "把握不足，仅保留摘要观察",
        "Kline long side unconfirmed": "K线多头结构未确认",
        "longFlow low but discovery evidence retained": "长周期资金不足，但启动证据仍需跟踪",
        "longFlow low downgrade": "长周期资金不支持，观察为主",
        "main momentum observe": "主流异动观察",
        "main momentum confirmed": "主流异动确认度较高",
        "duplicate merge": "同类信号短时间重复，合并观察",
        "risk conflicts with bullish evidence/flow": "风险触发但资金仍有支撑",
        "extreme risk unconfirmed observe": "极端风险未确认，先观察",
        "discovery potential observe": "启动迹象出现，等待确认",
        "bottom reversal observe": "风险释放但确认不足",
        "hot_breakout default observe/digest": "突破条件未充分确认",
        "low quality retained": "低质量但证据有亮点，保留跟踪",
        "观察层高分组合": "观察层高分组合",
        "追涨回撤风险": "追涨回撤风险",
    }
    parts: list[str] = []
    for raw_part in value.replace("；", ";").split(";"):
        ...
        if not part:
            continue
        if part.startswith("conviction ") and ">=" in part:
            continue
        if part.startswith("conviction ") and "<50" in part:
            parts.append("把握不足，等待更多确认")
            continue
        if part in replacements:
            parts.append(replacements[part])
            continue
        if any(token in part for token in ("digest", "observe", "realtime", "downgrade", "longFlow", "main momentum", "route_decision", "score only")):
            continue
        parts.append(part)
    cleaned = "；".join(dict.fromkeys(parts))
    return cleaned or "等待资金和K线确认"


def humanize_route_reason(
    route_decision: RouteDecision | None,
    signal: Signal | dict[str, Any] | None = None,
    context: dict[str, Any] | None = None,
) -> str:
    context = context or {}
    route = route_decision.route if route_decision else str(context.get("route") or "")
    tags = set(route_decision.route_tags if route_decision else [])
    data: dict[str, Any] = {}
    if signal is not None:
        snapshot = signal.snapshot if isinstance(signal, Signal) else None
        data = discord_route_metrics(signal, snapshot, context.get("price_action"))
    kind = topq_kind_normalized(data.get("kind") or (signal.kind if isinstance(signal, Signal) else ""))
    flow_label = str(data.get("flow_label") or "")
    evidence_summary = str(data.get("evidence_summary") or "")
    evidence_direction = str(data.get("evidence_direction") or "")
    conviction = int(data.get("conviction") or 0)
    leading = int(data.get("leading") or 0)
    evidence = int(data.get("evidence") or 0)
    price_action = data.get("price_action") or context.get("price_action")

    if "invalid" in tags or route == DISCORD_ROUTE_SUPPRESS:
        return "币种或数据无效，已过滤"
    ...
    if route == DISCORD_ROUTE_PRIORITY_OBSERVE:
        ...
        if "breakout_watch" in tags:
            return "低分信号但短中线结构转强，观察爆发延续"
        if "chase_pullback_risk" in tags:
            return "短线动能增强，但追涨回撤风险较高"
        if kind == "main_momentum_watch":
            return "主流异动观察；短线动能强；长周期未确认"
        if flow_label == "短强中弱":
            return "短线动能增强，但中长周期未确认"
        if flow_label == "资金分歧":
            return "资金分歧，先观察回踩承接"
        return "观察层高分组合，等待K线确认"
    if kind == "main_momentum_watch":
        if route == DISCORD_ROUTE_REALTIME:
            return "主流异动增强，资金和结构相对配合"
        return "主流异动观察；短线动能强；长周期未确认"
    if kind == "main_trend_watch":
        if route == DISCORD_ROUTE_REALTIME:
            return "主流趋势增强，但仍需留意回踩确认"
        return "主流趋势观察，等待资金和K线共振"
    if kind == "bottom_reversal":
        if route == DISCORD_ROUTE_REALTIME:
            return "底部承接增强，等待回踩不破确认"
        return "风险释放但确认不足，等待承接确认"
    if kind == "discovery":
        if route == DISCORD_ROUTE_REALTIME:
            return "启动迹象较强，资金和K线同步确认"
        if flow_label == "短强中弱":
            return "短线动能增强，但中长周期未确认"
        if text_has_any(evidence_summary, ("高位拥挤", "谨慎追", "不宜追", "追高风险")):
            return "启动迹象出现，但追高风险升高"
        return "启动迹象出现，等待K线和资金共振"
    if kind == "hot_breakout":
        return "突破条件未充分确认，不追高"
    if kind in DISCORD_RISK_KINDS or route == DISCORD_ROUTE_RISK_REALTIME:
        if evidence_direction == "看多" or text_has_any(evidence_summary, ("现货确认", "承接", "多周期共振流入")):
            return "风险触发但现货仍有支撑，暂不追空"
        if text_has_any(evidence_summary, ("高位拥挤", "注意出货", "派发")):
            return "多头拥挤且资金转弱，注意回撤"
        if route == DISCORD_ROUTE_RISK_REALTIME:
            return "顶部风险增强，等待跌破确认"
        return "风险触发但结构未确认，先观察"
    if "kline_downgrade" in tags or "longflow_downgrade" in tags:
        return "资金/结构未共振，观察为主"
    if route == DISCORD_ROUTE_DIGEST:
        if conviction >= 50 or leading >= 5 or evidence >= 6:
            return "分数低但证据有亮点，放入静默跟踪"
        return "确认不足，仅保留摘要观察"
    return clean_discord_reason(route_decision.reason if route_decision else "")
...
def humanize_action(
    ...
    if route == DISCORD_ROUTE_PRIORITY_OBSERVE:
        ...
        if "chase_pullback_risk" in tags:
            return "重点观察，等回踩确认，不追高"
        if kind == "main_momentum_watch":
            return "短线动能有，但大周期未确认，先等确认"
        return "重点观察，等回踩不破再考虑"
    if route == DISCORD_ROUTE_OBSERVE:
        if kind in DISCORD_RISK_KINDS:
            return "风险观察，等跌破短线结构"
        if price_action and text_has_any(price_action.label, ("箱体未突破", "大周期未确认")):
            return "等4h突破箱体确认"
        return "数据冲突，只放观察层"
    if route == DISCORD_ROUTE_DIGEST:
        return "不追高，等资金重新转强"
    ...
    if route == DISCORD_ROUTE_REALTIME:
        if kind in DISCORD_RISK_KINDS:
            return "顶部风险增强，先看减仓和跌破确认"
        return fallback_action or "等待回踩确认后再执行"
    cleaned_reason = clean_discord_reason(fallback_reason)
...
def discord_route_decision(
    ...
    if price_action is None and isinstance(signal, Signal) and signal.snapshot is not None and context.get("load_price_action", True):
        price_action = safe_multi_timeframe_price_action(signal.symbol, log_result=False)
    data = discord_route_metrics(signal, snapshot, price_action)
    symbol = data["symbol"]
    kind = data["kind"]
    ...
    net_flow_15m = data["net_flow_15m"]
    net_flow_1h = data["net_flow_1h"]
    ...
    if context.get("duplicate"):
        return RouteDecision(DISCORD_ROUTE_OBSERVE, "观察", "duplicate merge", max(0, conviction), True, ["duplicate"])

    if conviction >= 65:
        route = DISCORD_ROUTE_RISK_REALTIME if risk_kind else DISCORD_ROUTE_REALTIME
        reasons.append(f"conviction {conviction}>=65")
        tags.append("conviction")
    elif 50 <= conviction <= 64:
        if leading >= 5 or evidence >= 8 or discord_price_action_bullish_confirmed(price_action):
            route = DISCORD_ROUTE_OBSERVE
            reasons.append("conviction 50-64 kept by leading/evidence/Kline")
            tags.append("mid_conviction_keep")
        else:
            route = DISCORD_ROUTE_DIGEST
            reasons.append("conviction 50-64 digest")
            tags.append("mid_conviction_digest")
    else:
        route = DISCORD_ROUTE_DIGEST
        reasons.append(f"conviction {conviction}<50")
    ...
    if kind == "hot_breakout":
        hot_ok = (
            ...
            and trap <= 3
            and entry_score >= 8
            and (price_action is None or discord_price_action_bullish_confirmed(price_action))
            and not text_has_any(evidence_summary, ("高位拥挤", "不宜追", "谨慎追", "追高风险"))
        )
        route = DISCORD_ROUTE_REALTIME if hot_ok else (DISCORD_ROUTE_OBSERVE if conviction >= 50 else DISCORD_ROUTE_DIGEST)
        reasons.append("hot_breakout strict realtime" if hot_ok else "hot_breakout default observe/digest")
        tags.append("hot_breakout")
    elif kind == "discovery":
        discovery_ok = (
            conviction >= 70
            and leading >= 5
            and evidence >= 8
            and (price_action is None or price_action.mid_score >= 5)
            and flow_label not in TOPQ_WEAK_FLOW_LABELS
            and not text_has_any(evidence_summary, ("高位拥挤", "中长线资金不支持", "追高风险", "不宜追"))
        )
        route = DISCORD_ROUTE_REALTIME if discovery_ok else (DISCORD_ROUTE_OBSERVE if conviction >= 50 or leading >= 5 or evidence >= 8 else DISCORD_ROUTE_DIGEST)
        reasons.append("discovery realtime confirmed" if discovery_ok else "discovery potential observe")
        tags.append("discovery")
    elif kind == "bottom_reversal":
        bottom_realtime_confirmed = (
            discord_price_action_bottom_confirmed(price_action)
            or net_flow_15m > 0
            or net_flow_1h > 0
            or (leading >= 6 and trap <= 5)
        )
        bottom_ok = conviction >= 60 and trap <= 7 and bottom_realtime_confirmed
        route = DISCORD_ROUTE_REALTIME if bottom_ok else (DISCORD_ROUTE_OBSERVE if conviction >= 50 or evidence >= 8 else DISCORD_ROUTE_DIGEST)
        reasons.append("bottom reversal confirmed" if bottom_ok else "bottom reversal observe")
        tags.append("bottom_reversal")
    elif kind in {"top_risk", "top_exhaustion", "distribution"}:
        evidence_risk_confirmed = evidence_direction == "看空/风险" and evidence >= 5
        kline_risk_confirmed = price_action_structure_risk_confirmed(price_action)
        entry_risk_confirmed = (
            any(term in entry_label for term in ("逃顶", "减仓优先"))
            and trap <= 6
            and evidence_direction in {"看空/风险", "观察"}
            and evidence >= 5
            and flow_label not in {"短强中弱", "短弱中强"}
        )
        risk_ok = (
            conviction >= 65
            and (entry_risk_confirmed or kline_risk_confirmed or evidence_risk_confirmed)
        )
        if context.get("risk_realtime_proxy") and price_action is None and conviction >= 65 and evidence_risk_confirmed:
            risk_ok = True
            reasons.append("risk realtime proxy")
            tags.append("proxy")
        route = DISCORD_ROUTE_RISK_REALTIME if risk_ok else (DISCORD_ROUTE_OBSERVE if conviction >= 50 or evidence_direction in {"看空/风险", "观察"} else DISCORD_ROUTE_DIGEST)
        if evidence_direction == "看多" and flow_label in {"多周期共振流入", "中长线吸筹"} and not kline_risk_confirmed:
            route = DISCORD_ROUTE_OBSERVE
            reasons.append("risk conflicts with bullish evidence/flow")
        if long_flow == 0 and trap >= 8 and "高位出货" in str(discord_route_value(signal, "market_intent_label", "")) and not price_action_structure_risk_confirmed(price_action):
            route = DISCORD_ROUTE_OBSERVE
            reasons.append("extreme risk unconfirmed observe")
        reasons.append("risk realtime confirmed" if risk_ok else "risk observe")
        tags.append("risk")
    elif kind == "main_momentum_watch":
        momentum_ok = (
            conviction >= 65
            and flow_label not in {"资金分歧", "短强中弱", "短弱中强"}
            and (price_action is None or price_action.long_score > 3)
            and long_flow >= 5
            and evidence >= 8
        )
    ...
    if bullish_kind and price_action is not None and (
        ...
        route = discord_route_downgrade(route)
        reasons.append("Kline long side unconfirmed")
        tags.append("kline_downgrade")

    if bullish_kind and long_flow <= 3 and kind in {"discovery", "bottom_reversal", "hot_breakout", "main_momentum_watch", "main_trend_watch"}:
        if kind == "discovery" and (leading >= 6 or evidence >= 8 or discord_price_action_score_confirmed(price_action)):
            if route == DISCORD_ROUTE_REALTIME:
                route = DISCORD_ROUTE_PRIORITY_OBSERVE
            elif route == DISCORD_ROUTE_DIGEST:
                route = DISCORD_ROUTE_OBSERVE
            reasons.append("longFlow low but discovery evidence retained")
            tags.append("longflow_retained")
        else:
            route = discord_route_downgrade(route)
            reasons.append("longFlow low downgrade")
            tags.append("longflow_downgrade")

    quality = data["quality"]
    if quality < 40 and (conviction >= 60 or leading >= 6 or evidence >= 8 or kind in {"discovery", "bottom_reversal", "top_risk"}):
        if route == DISCORD_ROUTE_SUPPRESS:
            route = DISCORD_ROUTE_DIGEST
        reasons.append("low quality retained")
        tags.append("low_quality_retained")

    if route in {DISCORD_ROUTE_OBSERVE, DISCORD_ROUTE_DIGEST} and discord_priority_observe_candidate(data, risk_kind):
        route = DISCORD_ROUTE_PRIORITY_OBSERVE
        reasons.append("观察层高分组合")
        tags.append("priority_observe")
    if (
        route in {DISCORD_ROUTE_OBSERVE, DISCORD_ROUTE_DIGEST}
        and not context.get("disable_breakout_watch")
        and discord_breakout_watch_candidate(data)
    ):
        route = DISCORD_ROUTE_PRIORITY_OBSERVE
        reasons.append("低分信号但短中线结构转强，观察爆发延续")
        tags.append("breakout_watch")
    if (
        route in {DISCORD_ROUTE_OBSERVE, DISCORD_ROUTE_DIGEST}
        and not context.get("disable_breakout_watch")
        and discord_breakout_watch_fallback_candidate(data)
    ):
        route = DISCORD_ROUTE_PRIORITY_OBSERVE
        reasons.append("历史字段不足，但K线和OI结构转强，观察爆发延续")
        tags.append("breakout_watch")
        tags.append("breakout_watch_fallback")
...
def single_signal_direction_conflict_for_display(
    ...
    if isinstance(signal, Signal) and signal.snapshot is not None:
        data = discord_route_metrics(signal, signal.snapshot, price_action)
    elif isinstance(signal, dict):
        data = discord_route_metrics(signal, None, price_action)
    else:
        return False
    return single_signal_direction_conflict(data)


DISCORD_CONFLICT_OBSERVE_TAG = "conflict_observe"
DISCORD_CONFLICT_LONG_FORBIDDEN_KEYWORDS = (
    "高位拥挤",
    "派发",
    "主动卖盘强",
    "主动卖盘转强",
...
def discord_side_for_signal(
    signal: Signal | dict[str, Any] | None,
    route_decision: RouteDecision | None = None,
) -> str:
    if signal is None:
        return ""
    kind = topq_kind_normalized(signal.kind if isinstance(signal, Signal) else signal.get("kind"))
    parts: list[str] = [kind]
    if isinstance(signal, Signal):
        parts.extend([signal.title, signal.message])
    else:
        parts.extend(
            str(signal.get(key) or "")
            for key in (
    ...
    if route_decision:
        parts.append(route_decision.reason)
        parts.extend(route_decision.route_tags or [])
    text = " ".join(str(part or "") for part in parts)
    risk = (
        kind in DISCORD_RISK_KINDS
        or topq_is_risk_candidate(kind)
        or text_has_any(text, ("risk", "short", "风险", "看空", "减仓", "top_risk", "top_exhaustion", "distribution", "main_risk_watch"))
    )
    bullish = (
        kind in DISCORD_BULLISH_KINDS
        or topq_is_bullish_candidate(kind)
        or text_has_any(text, ("direction long", "long", "看多", "bottom_reversal", "discovery", "hot_breakout", "main_momentum_watch", "main_trend_watch"))
    )
    if risk and not bullish:
        return "risk"
    if bullish and not risk:
        return "long"
    if risk and bullish:
        return "risk" if kind in DISCORD_RISK_KINDS or topq_is_risk_candidate(kind) else "long"
    return ""
...
def discord_conflict_filter_long_evidence(items: list[str]) -> list[str]:
    return [item for item in items if item and not text_has_any(item, DISCORD_CONFLICT_LONG_FORBIDDEN_KEYWORDS)]


def discord_conflict_evidence_lists(
    signal: Signal | dict[str, Any] | None,
    previous: Signal | dict[str, Any] | None = None,
    price_action: MultiTimeframePriceAction | None = None,
) -> tuple[list[str], list[str]]:
    snapshots = []
    for item in (previous, signal):
        snapshot = item.snapshot if isinstance(item, Signal) else None
        if snapshot is not None:
            snapshots.append(snapshot)
    ...
    risk_items: list[str] = []
    if snapshot is not None:
        short_in = any(summary_flow_value(snapshot, period) > 0 for period in ("5m", "15m", "1h"))
        mid_in = any(summary_flow_value(snapshot, period) > 0 for period in ("4h", "12h", "24h"))
        long_in = any(summary_flow_value(snapshot, period) > 0 for period in ("72h", "144h"))
        if short_in or mid_in or long_in:
            labels = []
            if short_in:
                labels.append("短线")
            if mid_in:
            ...
            if long_in:
                labels.append("长线")
            long_items.append("/".join(labels) + "资金流入")
        if snapshot.taker_buy_sell_ratio is not None and snapshot.taker_buy_sell_ratio >= 1.08:
            long_items.append("主动买盘强")
        if snapshot.oi_change_percent > 0 and snapshot.price_change_percent > 0:
            long_items.append("OI增仓推涨")
        if snapshot.taker_buy_sell_ratio is not None and snapshot.taker_buy_sell_ratio < 1:
            risk_items.append("主动卖盘强")
            if snapshot.oi_change_percent > 0:
                risk_items.append("OI扩张但主动买卖比<1")
        if (
            (snapshot.global_long_short_ratio is not None and snapshot.global_long_short_ratio >= 1.8)
            or (snapshot.top_account_ratio is not None and snapshot.top_account_ratio >= 1.8)
        ):
            risk_items.append("散户多头拥挤")
    text_parts: list[str] = []
    for item in (previous, signal):
        if isinstance(item, Signal):
            text_parts.extend([item.title, item.message])
        elif isinstance(item, dict):
            text_parts.extend(str(item.get(key) or "") for key in ("evidence_summary", "message", "discord_route_reason", "external_confirmation_text"))
    combined = " ".join(str(part or "") for part in text_parts)
    if isinstance(signal, Signal) and signal.snapshot is not None:
        try:
            _ev_score, _ev_direction, ev_summary, ev_items = evidence_score(signal.snapshot, signal)
            combined = f"{combined} {ev_summary} " + " ".join(item.label for item in ev_items)
        except Exception:
            pass
        try:
            _short_flow, _mid_flow, _long_flow, flow_label, _flow_reason = flow_horizon_scores(signal.snapshot)
            combined = f"{combined} {flow_label}"
            if flow_label == "短强中弱":
                long_items.append("短线资金回流")
                risk_items.append("短强中弱，谨慎追")
                risk_items.append("中线资金不支持")
            elif flow_label == "资金分歧":
                risk_items.append("资金分歧，谨慎追")
        except Exception:
            pass
    elif isinstance(signal, dict):
        combined = f"{combined} {signal.get('flow_trend_label') or ''} {signal.get('kline_text') or ''}"
        if str(signal.get("flow_trend_label") or "") == "短强中弱":
            long_items.append("短线资金回流")
            risk_items.append("短强中弱，谨慎追")
            risk_items.append("中线资金不支持")
    if text_has_any(combined, ("主力建仓", "中线主力流入", "长线主力流入")):
        if "主力建仓" in combined:
            long_items.append("主力建仓")
        if text_has_any(combined, ("中线主力流入", "中线资金流入")):
            long_items.append("中线主力流入")
        if text_has_any(combined, ("长线主力流入", "长线资金流入")):
            long_items.append("长线主力流入")
    if text_has_any(combined, ("15m", "1h", "转强", "突破近20根高点", "放量阳线")) and text_has_any(combined, ("转强", "突破", "放量阳线")):
        long_items.append("15m/1h K线转强")
    if text_has_any(combined, ("现货承接", "现货/DEX/外部确认", "现货确认", "外部确认")):
        if "现货承接" in combined:
            long_items.append("现货承接")
        else:
            long_items.append("现货/外部确认")
    if text_has_any(combined, ("中线资金不支持", "中线不支持")):
        risk_items.append("中线资金不支持")
    if text_has_any(combined, ("长线资金不支持", "长线不支持", "中长线资金不支持")):
        risk_items.append("长线资金不支持")
    if text_has_any(combined, ("压力位", "接近压力区", "追高风险")):
        risk_items.append("接近压力区，追高风险")
    if text_has_any(combined, ("现货确认", "外部确认", "现货/DEX/外部确认")) and not text_has_any(combined, DISCORD_WEAK_EXTERNAL_CONFIRMATION_KEYWORDS):
        long_items.append("现货/外部确认")
    if text_has_any(combined, ("现货/外部确认不足", "外部确认不足", "现货确认偏弱", "外部确认偏弱", "现货未跟", "无标准现货", "偏弱")):
        risk_items.append("现货/外部确认不足")
    if price_action is not None:
        pa_text = " ".join([price_action.label, price_action.recommendation, *price_action.items, *price_action.risk_items, *price_action.patterns])
        if price_action.long_score <= 3 or text_has_any(pa_text, ("大周期下跌", "大周期未确认")):
            risk_items.append("大周期下跌趋势反弹")
            risk_items.append("长线资金不支持")
        if text_has_any(pa_text, ("4h箱体未突破", "箱体未突破")):
            risk_items.append("4h箱体未突破")
    long_items = discord_conflict_filter_long_evidence(long_items)
    if not long_items:
        long_items.append("短线/中线/长线资金流入")
    if not risk_items:
        risk_items.append("现货/外部确认不足")
    return list(dict.fromkeys(long_items))[:5], list(dict.fromkeys(risk_items))[:5]


def discord_conflict_observe_fields(
    signal: Signal,
    previous: Signal | None = None,
    route_decision: RouteDecision | None = None,
) -> list[tuple[str, str, bool]]:
    price_action = safe_multi_timeframe_price_action(signal.symbol, log_result=True) if signal.snapshot else None
    long_items, risk_items = discord_conflict_evidence_lists(signal, previous, price_action)
    single_conflict = bool(route_decision and "single_signal_direction_conflict" in (route_decision.route_tags or []))
    if single_conflict:
        try:
            _short_flow, _mid_flow, _long_flow, flow_label, _flow_reason = flow_horizon_scores(signal.snapshot) if signal.snapshot else (0, 0, 0, "", "")
        except Exception:
            flow_label = ""
        kline_text = ""
        if price_action is not None:
            kline_text = " ".join([price_action.label, price_action.recommendation, *price_action.items, *price_action.risk_items, *price_action.patterns])
        sui_style = flow_label == "短强中弱" or text_has_any(kline_text, ("15m", "1h", "转强", "突破", "放量阳线"))
        conclusion = (
            "短线结构转强，但中长线资金和大周期仍不支持，暂不站队。"
            if sui_style
            else "看多资金证据仍在，但短线卖压/压力位出现风险，暂不站队。"
        )
        action = (
            "不追多，也不急着追空。等待 15m/1h 方向和中线资金重新一致。"
            if sui_style
...
def discord_conflict_observe_embed(
    ...
    decision = route_decision or discord_conflict_route_decision()
    return DiscordOutboundMessage(
        channel_key=channel_key,
        title=f"🟡 多空分歧观察 {discord_symbol_pair(signal.symbol)}",
        color=DISCORD_COLOR_WATCH,
        fields=discord_conflict_observe_fields(signal, previous, decision),
        symbol=signal.symbol,
        kind="conflict_observe",
        final_route=decision.route,
        route_tags=decision.route_tags,
    )
...
def format_alt_watch_reason(text: str) -> str:
    compact = " ".join(clean_discord_reason(text).replace("；", " ").replace(";", " ").split())
...
def discord_alt_watch_item_from_row(row: dict[str, str], row_time: dt.datetime) -> DiscordAltWatchItem | None:
    ...
    if symbol in MAINSTREAM_WATCH_SYMBOLS:
        return None
    route = str(row.get("discord_route") or "").strip() or str(row.get("route") or "").strip()
    suppressed = str(row.get("suppressed_from_telegram") or "").strip()
    if route:
        if route not in {DISCORD_ROUTE_OBSERVE, DISCORD_ROUTE_DIGEST}:
            return None
    elif suppressed not in {"1", "true", "True"}:
        return None
    ...
    if not route and quality < 25 and conviction < 45:
        return None
    if not route and trap >= 8:
        return None
    if not route and not (conviction >= 50 or quality >= 45 or evidence >= 6 or leading >= 5):
        return None

    sort_score = int(parse_float(row.get("discord_route_score")) or conviction + quality + evidence * 3 + leading * 3)
    route_tags = [tag for tag in str(row.get("discord_route_tags") or "").split(";") if tag]
    decision = RouteDecision(route or DISCORD_ROUTE_OBSERVE, discord_route_level(route or DISCORD_ROUTE_OBSERVE), row.get("discord_route_reason") or "", sort_score, True, route_tags)
    return DiscordAltWatchItem(
        ...
        flow_label=str(row.get("flow_trend_label") or "-"),
        reason=format_alt_watch_reason(humanize_route_reason(decision, row)),
        sort_score=sort_score,
        route=route or DISCORD_ROUTE_OBSERVE,
    )
