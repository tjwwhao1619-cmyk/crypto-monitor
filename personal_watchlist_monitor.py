...
TOKEN_NAME = "LYN"
TOKEN_SYMBOL = "LYN"
TOKEN_ADDRESS = "0x302dfaf2cdbe51a18d97186a7384e87cf599877d"
LYN_PRICE_MOVE_ALERT_PCT = float(os.getenv("LYN_PRICE_MOVE_ALERT_PCT", "3.0"))
LYN_LIQUIDITY_DROP_ALERT_PCT = float(os.getenv("LYN_LIQUIDITY_DROP_ALERT_PCT", "15.0"))
LYN_CEX_DEX_SPREAD_ALERT_PCT = float(os.getenv("LYN_CEX_DEX_SPREAD_ALERT_PCT", "2.0"))
LYN_LARGE_TRANSFER_USD = float(os.getenv("LYN_LARGE_TRANSFER_USD", "10000"))
LYN_WHALE_CHECK_INTERVAL_SECONDS = int(os.getenv("LYN_WHALE_CHECK_INTERVAL_SECONDS", "60"))
LYN_WHALE_TRANSFER_USD = float(os.getenv("LYN_WHALE_TRANSFER_USD", "3000"))
LYN_WHALE_FLOW_IMBALANCE_USD = float(os.getenv("LYN_WHALE_FLOW_IMBALANCE_USD", "5000"))
LYN_WHALE_WINDOW_SECONDS = int(os.getenv("LYN_WHALE_WINDOW_SECONDS", "900"))
LYN_WHALE_REPEAT_USD = float(os.getenv("LYN_WHALE_REPEAT_USD", "5000"))
LYN_WHALE_ALERT_COOLDOWN_SECONDS = int(os.getenv("LYN_WHALE_ALERT_COOLDOWN_SECONDS", "180"))
LYN_PRIORITY_EVENT_USD = float(os.getenv("LYN_PRIORITY_EVENT_USD", "3000"))
LYN_PRIORITY_FLOW_IMBALANCE_USD = float(os.getenv("LYN_PRIORITY_FLOW_IMBALANCE_USD", "6000"))
LYN_ONE_HOP_TTL_SECONDS = int(os.getenv("LYN_ONE_HOP_TTL_SECONDS", "21600"))
LYN_ONE_HOP_MAX_WALLETS = int(os.getenv("LYN_ONE_HOP_MAX_WALLETS", "300"))
LYN_CEX_WALLETS = {a.strip().lower() for a in os.getenv("LYN_CEX_WALLETS", "").split(",") if a.strip()}
FORCE_SUMMARY_INTERVAL_SECONDS = int(os.getenv("LYN_FORCE_SUMMARY_INTERVAL_SECONDS", "300"))
LYNATE_PATH = Path("/opt/crypto-monitor/personal_watch_lyn_state.json")
LYN_WATCH_ADDRESSES_PATH = Path(os.getenv("LYN_WATCH_ADDRESSES_PATH", "/opt/crypto-monitor/lyn_watch_addresses.json"))
CEX_WALLET_CANDIDATES_BSC_PATH = Path(os.getenv("CEX_WALLET_CANDIDATES_BSC_PATH", "/opt/crypto-monitor/cex_wallet_candidates_bsc.json"))
LYN_GENERIC_CEX_EXCHANGES = {x.strip().lower() for x in os.getenv("LYN_GENERIC_CEX_EXCHANGES", "mexc,bingx").split(",") if x.strip()}
LYN_CEX_STRONG_CONFIDENCE = {"official", "confirmed"}
LYN_CEX_WEAK_CONFIDENCE = {"bscscan_tag", "observed", "heuristic", "candidate"}
LYN_CEX_STRONG_ROLES = {"deposit", "deposit_address", "hot_wallet", "aggregator"}
LYN_CEX_WEAK_ROLES = {"warm_wallet", "collector", "sweeper"}

DEX_URL = f"https://api.dexscreener.com/latest/dex/tokens/{TOKEN_ADDRESS}"
POLL_SECONDS = int(os.environ.get("LYN_WATCH_POLL_SECONDS", "30"))
SUMMARY_SECONDS = int(os.environ.get("LYN_WATCH_SUMMARY_SECONDS", "1800"))
PRICE_ALERT_PCT = float(os.environ.get("LYN_WATCH_PRICE_ALERT_PCT", "3.0"))
LIQ_ALERT_PCT = float(os.environ.get("LYN_WATCH_LIQ_ALERT_PCT", "8.0"))
LIQ_ALERT_USD = float(os.environ.get("LYN_WATCH_LIQ_ALERT_USD", "10000"))
VOLUME_M5_ALERT_USD = float(os.environ.get("LYN_WATCH_VOLUME_M5_ALERT_USD", "20000"))
TRANSFER_ALERT_USD = float(os.environ.get("LYN_WATCH_TRANSFER_ALERT_USD", "20000"))
...
def load_state():
    if LYNATE_PATH.exists():
        try:
            return json.loads(LYNATE_PATH.read_text(encoding="utf-8"))
        except Exception:
            pass
...
def save_state(state):
    tmp = LYNATE_PATH.with_suffix(".tmp")
    tmp.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")
    tmp.replace(LYNATE_PATH)
...
def load_lyn_watch_addresses():
    try:
        if LYN_WATCH_ADDRESSES_PATH.exists():
            data = json.loads(LYN_WATCH_ADDRESSES_PATH.read_text(encoding="utf-8"))
        else:
            data = {}
    except Exception:
        logging.exception("LYN watch address file load failed")
        data = {}
...
def load_cex_wallet_candidates_bsc():
    try:
        ...
        logging.exception("LYN generic CEX candidate file load failed")
        return {}
    ...
    for row in rows if isinstance(rows, list) else []:
        ...
        if exchange.lower() not in LYN_GENERIC_CEX_EXCHANGES:
            continue
        ...
        strength = "weak"
        if confidence.lower() in LYN_CEX_STRONG_CONFIDENCE:
            strength = "strong_confirmed"
        elif role.lower() in LYN_CEX_STRONG_ROLES and confidence.lower() in LYN_CEX_WEAK_CONFIDENCE:
            strength = "strong_structural"
        elif confidence.lower() in LYN_CEX_WEAK_CONFIDENCE or role.lower() in LYN_CEX_WEAK_ROLES:
            strength = "weak"
        wallet_map[addr] = {
            "exchange": exchange,
            "role": role,
...
def check_events(state):
    pairs = fetch_dex()
    if not pairs:
        alert_once(state, "dex_no_data", "⚠️ LYN 持仓监控：DEX暂无数据", f"{TOKEN_ADDRESS}", color=0xE67E22, cooldown=900)
        return
    ...
    if last_price:
        ...
        if abs(move) >= PRICE_ALERT_PCT:
            ...
            title = f"{'🟢' if move > 0 else '🔴'} LYN 价格异动 {move:+.2f}%"
    ...
    if last_liq:
        ...
        if abs(diff) >= LIQ_ALERT_USD or abs(diff_pct) >= LIQ_ALERT_PCT:
            ...
            title = f"{'🟢' if diff > 0 else '🔴'} LYN DEX流动性{'增加' if diff > 0 else '减少'}"
            ...
            if last:
                ...
                if abs(move) >= PRICE_ALERT_PCT:
                    alert_once(
                        ...
                        f"{'🟢' if move > 0 else '🔴'} LYN {x['exchange']}现货异动 {move:+.2f}%",
    ...
    for tr in transfers:
        ...
        alert_once(
            state,
            f"transfer:{h}",
            f"🐳 LYN 链上大额转账 {money(tr['usd'])}",
            f"{fnum(tr['amount'])} LYN | {short_addr(tr['from'])} -> {short_addr(tr['to'])}\n{tr['source']} tx {h}",
            fields,
            color=0x9B59B6,
            cooldown=60,
        )
    ...
    if ts - float(state.get("last_summary", 0) or 0) >= SUMMARY_SECONDS:
        ...
        front_row_field = _lyn_front_row_summary_field(state)
        if front_row_field:
...
def _lyn_whale_short(addr):
    addr = (addr or "").lower()
    return addr[:6] + "..." + addr[-4:] if len(addr) > 12 else addr


def _lyn_whale_amount(row):
    try:
...
def _lyn_whale_fetch_pairs(state):
    now = time.time()
    if now - float(state.get("lyn_pair_cache_ts", 0) or 0) < 300 and state.get("lyn_dex_pair_addresses"):
        return {str(x).lower() for x in state.get("lyn_dex_pair_addresses", [])}
    try:
        ...
        for p in pairs:
            ...
            if pair_addr:
                ...
                labels.append(f"{p.get('dexId','dex')}:{_lyn_whale_short(pair_addr)}")
        state["lyn_dex_pair_addresses"] = sorted(set(addrs))
        state["lyn_pair_cache_ts"] = now
        state["lyn_dex_pair_labels"] = labels[:8]
        return set(state["lyn_dex_pair_addresses"])
    except Exception:
        logging.exception("LYN whale pair fetch failed")
        return {str(x).lower() for x in state.get("lyn_dex_pair_addresses", [])}


def _lyn_whale_fetch_transfers(limit=50):
    ...
    try:
        ...
        if data.get("status") != "1" or not isinstance(data.get("result"), list):
            logging.info("LYN whale transfer fetch skipped: %s", str(data)[:300])
            return []
        return data["result"]
    except Exception:
        logging.exception("LYN whale transfer fetch failed")
        return []


def _lyn_whale_price(state):
    prices = state.get("prices", {}) if isinstance(state, dict) else {}
...
def _cleanup_lyn_one_hop_watch(state, now_ts=None):
    now_ts = int(now_ts or time.time())
    watch = state.setdefault("lyn_one_hop_watch", {})
    fresh = {
        str(addr).lower(): _normalize_lyn_one_hop_meta(meta, now_ts)
    ...
    if len(fresh) > LYN_ONE_HOP_MAX_WALLETS:
        ranked = sorted(
            fresh.items(),
            key=lambda item: (
                int((item[1] or {}).get("last_seen_ts", 0) or 0),
                int((item[1] or {}).get("hit_count", 0) or 0),
            ),
            reverse=True,
        )
        fresh = dict(ranked[:LYN_ONE_HOP_MAX_WALLETS])
    state["lyn_one_hop_watch"] = fresh
    return fresh


def _normalize_lyn_one_hop_meta(meta, now_ts=None):
    now_ts = int(now_ts or time.time())
    meta = dict(meta or {})
    ...
    return {
        ...
        "expires_at": int(meta.get("expires_at", last_seen_ts + LYN_ONE_HOP_TTL_SECONDS) or (last_seen_ts + LYN_ONE_HOP_TTL_SECONDS)),
...
def _infer_lyn_one_hop_source_confidence(meta):
    meta = dict(meta or {})
    ...
    if origin_type in {"core_top_holder", "top_holder", "accumulator", "distributor"}:
        return "confirmed"
...
def _track_lyn_one_hop_wallet(state, addr, origin_type, origin_label, origin_address, now_ts=None, source_confidence=""):
    addr = (addr or "").lower()
    if not addr:
        return None
    watch = _cleanup_lyn_one_hop_watch(state, now_ts)
    now_ts = int(now_ts or time.time())
    meta = _normalize_lyn_one_hop_meta(watch.get(addr) or {}, now_ts)
    meta.update(
        {
            "last_seen_ts": now_ts,
            "expires_at": now_ts + LYN_ONE_HOP_TTL_SECONDS,
    ...
    meta["source_confidence"] = _infer_lyn_one_hop_source_confidence(meta)
    watch[addr] = meta
    state["lyn_one_hop_watch"] = watch
    return meta


def _lyn_one_hop_meta(state, addr):
    addr = (addr or "").lower()
    watch = _cleanup_lyn_one_hop_watch(state)
    meta = watch.get(addr)
    if not isinstance(meta, dict):
        return None
    meta = _normalize_lyn_one_hop_meta(meta)
    inferred = _infer_lyn_one_hop_source_confidence(meta)
...
def _lyn_update_one_hop_event(state, actor, side, escalated=False):
    actor = (actor or "").lower()
    if not actor:
        return None
    meta = _lyn_one_hop_meta(state, actor)
    if not meta:
        return None
    meta = _normalize_lyn_one_hop_meta(meta)
    ...
    meta["source_confidence"] = _infer_lyn_one_hop_source_confidence(meta)
    state.setdefault("lyn_one_hop_watch", {})[actor] = meta
    return meta


def _lyn_front_row_summary(window, now_ts=None):
    now_ts = int(now_ts or time.time())
    cutoff = now_ts - LYN_WHALE_WINDOW_SECONDS
    scoped = [x for x in (window or []) if int(x.get("ts", 0) or 0) >= cutoff]
    summary = {
        "window_minutes": int(LYN_WHALE_WINDOW_SECONDS / 60),
...
def _lyn_front_row_summary_field(state):
    summary = state.get("lyn_front_row_summary") or {}
    if not summary:
        return None
    return (
        "前排15m摘要",
...
def lyn_address_label(addr, labels=None, state=None):
    addr = (addr or "").lower()
    labels = labels or load_lyn_watch_addresses()
    ...
    if state is not None:
        one_hop_meta = _lyn_one_hop_meta(state, addr)
        if one_hop_meta:
...
def _lyn_whale_side(row, pair_addrs):
    labels = load_lyn_watch_addresses()
    state = row.get("_state") or {}
    now_ts = int(row.get("timeStamp") or time.time())
    src = (row.get("from") or "").lower()
    dst = (row.get("to") or "").lower()

    src_type, src_label, src_meta = lyn_address_label(src, labels, state)
    dst_type, dst_label, dst_meta = lyn_address_label(dst, labels, state)

    if src_type in {"core_top_holder", "accumulator", "distributor", "cex"} and dst_type == "unknown":
        _track_lyn_one_hop_wallet(state, dst, src_type, src_label, src, now_ts, source_confidence=str(src_meta.get("strength") or src_type))
    if dst_type in {"core_top_holder", "accumulator", "distributor", "cex"} and src_type == "unknown":
        _track_lyn_one_hop_wallet(state, src, dst_type, dst_label, dst, now_ts, source_confidence=str(dst_meta.get("strength") or dst_type))

    one_hop_src = src_type == "one_hop_candidate"
    one_hop_dst = dst_type == "one_hop_candidate"
    front_row = src_type in {"core_top_holder", "top_holder"} or dst_type in {"core_top_holder", "top_holder"}
    ...
    def cex_label(base_label, meta):
        ...
        return f"{base_label} [{exchange}/{role}/{confidence}/{tier}]"

    if one_hop_src and (dst_type == "dex_pair" or dst in pair_addrs):
        return "sell", src, f"{src_label} 转入{dst_label or 'DEX池'}，一跳后进入 DEX，卖压权重上调", {"front_row": front_row, "one_hop": True, "cex_strength": "", "one_hop_escalated": True}
    if one_hop_dst and (src_type == "dex_pair" or src in pair_addrs):
        return "buy", dst, f"从{src_label or 'DEX池'}转入{dst_label}，一跳后承接 DEX，吸筹权重上调", {"front_row": front_row, "one_hop": True, "cex_strength": "", "one_hop_escalated": True}
    if dst_type == "dex_pair" or dst in pair_addrs:
        return "sell", src, f"转入{dst_label or 'DEX池'}，疑似出货/卖压", {"front_row": front_row, "one_hop": one_hop_src, "cex_strength": "", "one_hop_escalated": False}
    if src_type == "dex_pair" or src in pair_addrs:
        return "buy", dst, f"从{src_label or 'DEX池'}转出，疑似扫货/吸筹", {"front_row": front_row, "one_hop": one_hop_dst, "cex_strength": "", "one_hop_escalated": False}
    if dst_type == "cex":
        return "cex_deposit", src, f"转入{cex_label(dst_label, dst_meta)}，潜在卖压", {"front_row": front_row, "one_hop": one_hop_src, "cex_strength": dst_meta.get("strength") or "", "one_hop_escalated": one_hop_src}
    if src_type == "cex":
        return "cex_withdraw", dst, f"从{cex_label(src_label, src_meta)}提币，偏吸筹/囤币观察", {"front_row": front_row, "one_hop": one_hop_dst, "cex_strength": src_meta.get("strength") or "", "one_hop_escalated": one_hop_dst}
    if src_type == "core_top_holder" or dst_type == "core_top_holder":
        ...
        return "core_top_holder", actor, f"{label} 出现链上动作，最高优先级观察", {"front_row": True, "one_hop": one_hop_src or one_hop_dst, "cex_strength": "", "one_hop_escalated": False}
    if src_type == "top_holder" or dst_type == "top_holder":
        ...
        return "top_holder", actor, f"{label} 出现链上动作，重点观察", {"front_row": True, "one_hop": one_hop_src or one_hop_dst, "cex_strength": "", "one_hop_escalated": False}
    if src_type == "accumulator" or dst_type == "accumulator":
        actor = src if src_type == "accumulator" else dst
        return "accumulator", actor, "净流入吸筹候选地址出现动作，重点观察", {"front_row": front_row, "one_hop": one_hop_src or one_hop_dst, "cex_strength": "", "one_hop_escalated": False}
    if src_type == "distributor" or dst_type == "distributor":
        actor = src if src_type == "distributor" else dst
        return "distributor", actor, "净流出派发候选地址出现动作，风险观察", {"front_row": front_row, "one_hop": one_hop_src or one_hop_dst, "cex_strength": "", "one_hop_escalated": False}
    if src_type == "hub" or dst_type == "hub":
        actor = src if src_type == "hub" else dst
        return "hub", actor, "高周转/做市/归集候选地址动作，暂不判断方向", {"front_row": front_row, "one_hop": one_hop_src or one_hop_dst, "cex_strength": "", "one_hop_escalated": False}
    if src_type == "cex_candidate":
        return "transfer", dst, f"来自{cex_label(src_label, src_meta)}，交易所候选相关地址动作，暂不直接判定提币", {"front_row": front_row, "one_hop": one_hop_dst, "cex_strength": src_meta.get('strength') or "", "one_hop_escalated": False}
    if dst_type == "cex_candidate":
        return "transfer", src, f"转入{cex_label(dst_label, dst_meta)}，交易所候选相关地址动作，暂不直接判定充值", {"front_row": front_row, "one_hop": one_hop_src, "cex_strength": dst_meta.get('strength') or "", "one_hop_escalated": False}
    return "transfer", dst, "普通大额链上转账，暂不判断方向", {"front_row": front_row, "one_hop": one_hop_src or one_hop_dst, "cex_strength": "", "one_hop_escalated": False}


def _lyn_priority_bucket(event):
    side = event.get("side")
    usd = float(event.get("usd", 0) or 0)
    if usd < LYN_PRIORITY_EVENT_USD:
        return None
    label = str(event.get("label") or "")
    front_row = "核心前排钱包" in label or "前排持仓地址" in label
    ...
    if side in {"sell", "distributor", "cex_deposit"}:
        return "sell_pressure"
    if side in {"buy", "accumulator", "cex_withdraw"}:
        return "buy_pressure"
    return None


def _lyn_priority_flow_score(event):
    side = event.get("side")
    usd = float(event.get("usd", 0) or 0)
    label = str(event.get("label") or "")
    front_row = "核心前排钱包" in label or "前排持仓地址" in label
    ...
    if side == "buy":
        return usd * (1.5 if front_row else 1.0) * one_hop_factor
    if side == "cex_withdraw":
        return usd * (1.3 if front_row else 0.8) * one_hop_factor
    if side == "accumulator":
        return usd * 0.8
    if side == "sell":
        return -usd * (1.5 if front_row else 1.0) * one_hop_factor
...
def _lyn_whale_cooldown_ok(state, key):
    now = time.time()
    cooldowns = state.setdefault("lyn_whale_alert_cooldowns", {})
    last = float(cooldowns.get(key, 0) or 0)
    if now - last < LYN_WHALE_ALERT_COOLDOWN_SECONDS:
        return False
    cooldowns[key] = now
    return True
...
def check_whale_activity(state):
    now = time.time()
    if now - float(state.get("lyn_last_whale_check_ts", 0) or 0) < LYN_WHALE_CHECK_INTERVAL_SECONDS:
        return
    state["lyn_last_whale_check_ts"] = now

    price = _lyn_whale_price(state)
    if not price:
        logging.info("LYN whale check skipped: no price")
        return

    pair_addrs = _lyn_whale_fetch_pairs(state)
    rows = _lyn_whale_fetch_transfers(limit=50)
    if not rows:
        return

    latest_hashes = [r.get("hash") for r in rows if r.get("hash")]
    if not state.get("lyn_whale_monitor_initialized"):
        state["lyn_seen_whale_transfer_hashes"] = latest_hashes[-200:]
        state["lyn_whale_monitor_initialized"] = True
        discord_send(
            "👁️ LYN 大户行为监控已启动",
            f"已识别 DEX 池子 {len(pair_addrs)} 个；后续只提醒新发生的大额扫货/出货。\n阈值：单笔 {money(LYN_WHALE_TRANSFER_USD)}，窗口 {int(LYN_WHALE_WINDOW_SECONDS/60)} 分钟。",
            [("DEX池", "\n".join(state.get("lyn_dex_pair_labels", [])[:6]) or "-", False)],
            color=0x5865F2,
        )
        return

    seen = set(state.get("lyn_seen_whale_transfer_hashes") or [])
    new_events = []
    for row in reversed(rows):
        txh = row.get("hash")
        if not txh or txh in seen:
            continue
        amount = _lyn_whale_amount(row)
        usd = amount * price
        row["_state"] = state
        side, actor, label, tags = _lyn_whale_side(row, pair_addrs)
        event = {
        ...
        seen.add(txh)
        if event["one_hop"]:
            _lyn_update_one_hop_event(state, actor, side, escalated=event["one_hop_escalated"])

    logging.info(
        "LYN whale radar heartbeat: price=%s pairs=%s rows=%s new_events=%s initialized=%s",
        price,
        len(pair_addrs),
        len(rows),
        len(new_events),
        state.get("lyn_whale_monitor_initialized"),
    )
    ...
    fields = []
    for e in new_events:
        if e["usd"] >= LYN_WHALE_TRANSFER_USD:
            fields.append((
                f"{money(e['usd'])} / {fnum(e['amount'])} LYN",
                f"{e['label']}\n{_lyn_whale_short(e['from'])} -> {_lyn_whale_short(e['to'])}\nTx: `{e['hash']}`",
                False,
            ))

    if fields and _lyn_whale_cooldown_ok(state, "large_tx"):
        discord_send(
            "🟠 LYN 大户链上动作",
            f"发现 {len(fields)} 笔超过 {money(LYN_WHALE_TRANSFER_USD)} 的新链上动作。",
            fields[:6],
            color=0xFEE75C,
        )
    ...
    if priority_fields:
        if "front_row_risk" in priority_buckets and _lyn_whale_cooldown_ok(state, "priority_front_row_risk"):
            discord_send(
                "🔴 LYN 前排钱包风险动作",
                "检测到前排钱包相关的卖压/交易所流入动作，优先级高于普通大额转账。",
                priority_fields[:6],
                color=0xED4245,
            )
        elif "front_row_accum" in priority_buckets and _lyn_whale_cooldown_ok(state, "priority_front_row_accum"):
            discord_send(
                "🟢 LYN 前排钱包吸筹动作",
                "检测到前排钱包相关的扫货/交易所提币动作，优先级高于普通大额转账。",
                priority_fields[:6],
                color=0x57F287,
            )
        elif "front_row_active" in priority_buckets and _lyn_whale_cooldown_ok(state, "priority_front_row_active"):
            discord_send(
                "🟠 LYN 前排钱包活跃",
                "检测到前排钱包发生链上动作，建议结合 DEX/CEX 流向一起看。",
                priority_fields[:6],
                color=0xFEE75C,
            )
        elif "one_hop_risk" in priority_buckets and _lyn_whale_cooldown_ok(state, "priority_one_hop_risk"):
            discord_send(
                "🔴 LYN one-hop 风险动作",
                "检测到重点地址衍生的一跳地址流向 DEX/卖压路径，优先级高于普通转账。",
                priority_fields[:6],
                color=0xED4245,
            )
        elif "one_hop_accum" in priority_buckets and _lyn_whale_cooldown_ok(state, "priority_one_hop_accum"):
            discord_send(
                "🟢 LYN one-hop 吸筹动作",
                "检测到重点地址衍生的一跳地址承接 DEX/提币路径，优先级高于普通转账。",
                priority_fields[:6],
                color=0x57F287,
            )
        elif "sell_pressure" in priority_buckets and _lyn_whale_cooldown_ok(state, "priority_sell"):
            discord_send(
                "🔴 LYN DEX卖压动作",
                "检测到大额 LYN 持续流入 DEX 池或流向分发路径，优先关注卖压。",
                priority_fields[:6],
                color=0xED4245,
            )
        elif "buy_pressure" in priority_buckets and _lyn_whale_cooldown_ok(state, "priority_buy"):
            discord_send(
                "🟢 LYN DEX扫货动作",
                "检测到大额 LYN 从 DEX 池流出或流入吸筹路径，优先关注扫货。",
                priority_fields[:6],
                color=0x57F287,
            )

    window = state.get("lyn_whale_flow_window") or []
    cutoff = int(now - LYN_WHALE_WINDOW_SECONDS)
    window = [x for x in window if int(x.get("ts", 0)) >= cutoff]
    for e in new_events:
        if e["side"] in {"buy", "sell", "hub", "core_top_holder", "top_holder", "accumulator", "distributor", "cex_deposit", "cex_withdraw"}:
            window.append({k: e[k] for k in ("ts", "hash", "side", "actor", "amount", "usd", "front_row", "one_hop", "cex_strength")})
    state["lyn_whale_flow_window"] = window[-200:]
    state["lyn_front_row_summary"] = _lyn_front_row_summary(state["lyn_whale_flow_window"], now)
    ...
    if net >= LYN_WHALE_FLOW_IMBALANCE_USD and _lyn_whale_cooldown_ok(state, "flow_buy"):
        discord_send(
            "🟢 LYN 大户扫货占优",
            f"最近 {int(LYN_WHALE_WINDOW_SECONDS/60)} 分钟：扫货 {money(buy_usd)} / 出货 {money(sell_usd)}，净扫货 {money(net)}。",
            [("含义", "DEX 池子里的 LYN 被大额拿走，偏吸筹/扫货观察。", False)],
            color=0x57F287,
        )
    elif -net >= LYN_WHALE_FLOW_IMBALANCE_USD and _lyn_whale_cooldown_ok(state, "flow_sell"):
        discord_send(
            "🔴 LYN 大户出货占优",
            f"最近 {int(LYN_WHALE_WINDOW_SECONDS/60)} 分钟：出货 {money(sell_usd)} / 扫货 {money(buy_usd)}，净出货 {money(-net)}。",
            [("含义", "LYN 被持续打入 DEX 池子，卖压上升。", False)],
            color=0xED4245,
        )
    ...
    for (side, actor), stat in wallet_stats.items():
        if stat["count"] >= 2 and stat["usd"] >= LYN_WHALE_REPEAT_USD:
            label = "连续扫货" if side == "buy" else "连续出货"
            repeat_fields.append((label, f"{_lyn_whale_short(actor)} | {stat['count']} 笔 | {money(stat['usd'])}", False))

    if repeat_fields and _lyn_whale_cooldown_ok(state, "repeat_wallet"):
        discord_send(
            "👀 LYN 同地址连续动作",
            "同一地址在短窗口内连续买/卖，值得盯。",
            repeat_fields[:6],
    ...
    priority_window = state.get("lyn_priority_flow_window") or []
    priority_window = [x for x in priority_window if int(x.get("ts", 0)) >= cutoff]
    for e in new_events:
        score = _lyn_priority_flow_score(e)
        if score:
            priority_window.append({"ts": e["ts"], "score": score})
    state["lyn_priority_flow_window"] = priority_window[-200:]
    priority_net = sum(float(x.get("score", 0) or 0) for x in priority_window)
    if priority_net >= LYN_PRIORITY_FLOW_IMBALANCE_USD and _lyn_whale_cooldown_ok(state, "priority_flow_buy"):
        discord_send(
            "🟢 LYN 短窗净扫货占优",
            f"最近 {int(LYN_WHALE_WINDOW_SECONDS/60)} 分钟，大额地址净扫货 {money(priority_net)}。",
            [("判断", "更多 LYN 被从 DEX 池拿走，偏吸筹。", False)],
            color=0x57F287,
        )
    elif -priority_net >= LYN_PRIORITY_FLOW_IMBALANCE_USD and _lyn_whale_cooldown_ok(state, "priority_flow_sell"):
        discord_send(
            "🔴 LYN 短窗净卖压占优",
            f"最近 {int(LYN_WHALE_WINDOW_SECONDS/60)} 分钟，大额地址净卖压 {money(-priority_net)}。",
            [("判断", "更多 LYN 被持续打入 DEX 池，偏卖压。", False)],
            color=0xED4245,
        )

    state["lyn_seen_whale_transfer_hashes"] = list(seen)[-300:]
...
def check_holding_alerts(state):
    ...
    if dex_price and previous_price:
        ...
        if chg is not None and abs(chg) >= LYN_PRICE_MOVE_ALERT_PCT:
            direction = "上涨" if chg > 0 else "下跌"
            discord_send(
                f"⚠️ LYN 价格{direction}提醒",
    ...
    if total_liq and previous_liq:
        ...
        if chg is not None and chg <= -LYN_LIQUIDITY_DROP_ALERT_PCT:
            discord_send(
                "🔴 LYN 流动性下降提醒",
    ...
    if dex_price and cex_prices:
        ...
        for name, price in sorted(cex_prices.items()):
            ...
            if spread is not None and abs(spread) >= LYN_CEX_DEX_SPREAD_ALERT_PCT:
...
def send_summary(state, reason="定时持仓快照"):
    ...
    amount = float(os.getenv("LYN_WATCH_AMOUNT", "0") or 0)
    entry_price = float(os.getenv("LYN_ENTRY_PRICE", "0") or 0)
...
def main():
    load_env_file()
    if os.environ.get("LYN_WATCH_ENABLED", "1").lower() in {"0", "false", "off", "no"}:
        print("LYN holding monitor disabled")
        return

    state = load_state()
    prune_cex_prices(state)
    discord_send("✅ LYN 个人持仓监控已启动", f"{TOKEN_NAME} ({TOKEN_SYMBOL})\n`{TOKEN_ADDRESS}`\n频道: <#{os.environ.get('DISCORD_HOLDING_CHANNEL_ID','')}>", color=0x2ECC71)
    ...
    while True:
        try:
            ...
            if now_ts - float(state.get("last_forced_summary_ts", 0) or 0) >= FORCE_SUMMARY_INTERVAL_SECONDS:
                ...
                liquidity = state.get("liquidity", {})
                logging.info(
                    "LYN holding heartbeat: dex_best=%s cex=%s liquidity=%s pairs=%s",
                    prices.get("dex_best"),
            ...
            traceback.print_exc()
            try:
                alert_once(state, "loop_error", "⚠️ LYN 持仓监控异常", str(e)[:1500], color=0xE74C3C, cooldown=600)
                save_state(state)
            except Exception:
                pass
